...
class AudioController extends TypeController {
  ...
  async removeProcessor(id) {
    ...
    if (processor) {
      ...
      const taskId = (
        audioConfig.mixing ||
        audioConfig.transcoding ||
        audioConfig.selecting
      ).id;
  ...
  async createSession(direction, sessionConfig) {
    ...
    if (direction === "in") {
      ...
      const participant =
        processor.type === "atranscoder"
          ? format.codec
          : sessionConfig.participant;
      // output = "streamId"
      const outputId = await this.makeRPC(processor.locality.node, "generate", [
        participant,
        audioFormatStr(format),
      ]);
      ...
      const audioTrack = Object.assign(
        { id: outputId },
        sessionConfig.media.audio
      );
      publication.source.audio.push(audioTrack);
    ...
    if (this.sessions.has(id)) {
      ...
      if (session.direction === "in") {
        ...
        this.makeRPC(processor.locality.node, "degenerate", [id]).catch((e) =>
          log.debug("ignore degenerate callback")
        );
        const idx = processor.outputs.audio.findIndex(
          (track) => track.id === id
        );
        if (idx >= 0) {
