...
class AnalyticsController extends TypeController {
  ...
  async removeProcessor(id) {
    ...
    if (processor) {
      ...
      // Recycle node
      this.recycleWorkerNode(locality, procConfig.domain, procConfig.id).catch(
        (err) => log.debug("Failed to recycleNode:", err)
      );
    } else {
  ...
  async removeSession(id, direction, reason) {
    if (this.sessions.has(id)) {
      ...
      if (direction === "in") {
        ...
        this.makeRPC(processor.locality.node, "unsubscribe", [inputId]).catch(
          (e) => log.debug("ignore unpublish callback")
        );
        const idx = processor.inputs.audio.findIndex(
          (track) => track.id === id
        );
        if (idx >= 0) {
  ...
  onSessionProgress(id, type, data) {
    switch (type) {
      case "onStreamAdded": {
        ...
        this.createSession("in", outputConfig).catch((e) =>
          log.debug("Failed to create session on progress:", e)
        );
        break;
      }
      case "onStreamRemoved": {
        log.debug("onStreamRemoved:", id, type, data);
        this.removeSession(id, "in", "onStreamRemoved").catch((e) =>
          log.debug("Failed to remove session on progress:", e)
        );
        break;
      }
      default:
