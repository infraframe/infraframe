...
class ServiceScheduler {
  ...
  async scheduleService(req) {
    ...
    const scheduled = await this.stateStores.read("scheduleMaps", {
      _id: hash,
    });
    if (scheduled) {
  ...
  checkService() {
    ...
    this.stateStores
      .readMany("streamEngineNodes", {})
      .then(async (ret) => {
        const serviceNodes = ret.data || [];
        const req = { query: { _id: "non-existent" } };
        for (const service of serviceNodes) {
          try {
            await this.rpcChannel.makeRPC(service.id, "getNodes", [req]);
    ...
    for (const method of ServiceScheduler.supportedMethods) {
      api[method] = async function (req, callback) {
        ...
        try {
          ...
          const ret = await self.rpcChannel.makeRPC(serviceNode, method, [req]);
