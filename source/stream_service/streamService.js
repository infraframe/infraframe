...
const rpcID =
  config.service.name || "streamservice-" + randomUUID().replace(/-/g, "");
const schedulerID =
  config.scheduler && (config.scheduler.name || "stream-service");
...
function streamEngine(rpcClient) {
  ...
  const scheduler = startScheduler
    ? new ServiceScheduler(rpcChannel, stateStores)
    : null;
  ...
  const notifyPortal = function (endpoints, event, data) {
    ...
    if (endpoints.to) {
      stateStores
        .read("participants", { id: endpoints.to })
        .then((user) => {
      ...
      stateStores
        .read("domains", { id: endpoints.domain })
        .then((dm) => {
          log.debug("domain:", dm);
          const excludes = [];
          if (endpoints.from) {
          ...
          for (const portal of dm.portals) {
            ...
            rpcReq
              .broadcast(portal, controller, excludes, event, data)
              .catch((e) => {
  ...
  const removeParticipant = async function (id) {
    ...
    if (user && removed) {
      ...
      for (const subId of user.subs) {
        const prom = stateStores
          .read("subscriptions", { id: subId })
          .then((sub) => {
            if (sub) {
  ...
  const saveWorkerNode = async function (locality, type) {
    const node = new State.WorkerNode(locality.node, locality.agent, type);
    try {
  ...
  const getInternalAddress = async function (nodeId) {
    ...
    const addr = await rpcChannel.makeRPC(nodeId, "getInternalAddress", []);
  ...
  const linkSubscription = async function (subscription) {
    ...
    for (const type in subscription.sink) {
      for (const track of subscription.sink[type]) {
        ...
        const prom = stateStores
          .read("publications", { id: track.from })
          .then(async (pub) => {
            if (pub) {
            ...
            const srcTrack = await stateStores.read("sourceTracks", {
              id: track.from,
            });
            if (!srcTrack?.address) {
              throw new Error(`Failed to get link address for ${track.from}`);
            }
            const linkAddr = Object.assign(
              { id: track.from },
              srcTrack.address
            );
    ...
    for (const [trackId, fromInfo] of links) {
      ...
      linkPros.push(rpcChannel.makeRPC(subNode, "linkup", [trackId, fromInfo]));
    }
  ...
  const cutSubscription = async function (subscription) {
    ...
    for (const type in subscription.sink) {
      subscription.sink[type].forEach((track) => {
        ...
        const p1 = stateStores
          .setRemove("sourceTracks", { id: track.from }, subRemove)
          .catch((e) => {
        ...
        const p2 = rpcChannel
          .makeRPC(subNode, "cutoff", [track.id])
          .catch((reason) => {
  ...
  const removePublication = function (id) {
    ...
    stateStores
      ...
      .then(async (publication) => {
        ...
        stateStores
          .setRemove("participants", { id: owner }, { pubs: id })
          .catch(stateErrorHandler);
        stateStores
          .setRemove("nodes", { id: nodeId }, { pubs: id })
          .catch(stateErrorHandler);
        stateStores
          .delete("sourceTracks", { parent: id })
          .catch(stateErrorHandler);
  ...
  const updatePublication = function (updates) {
    ...
    stateStores
      .read("publications", { id: updates.id })
      .then(async (publication) => {
        if (!publication) {
  ...
  const removeSubscription = function (id) {
    ...
    stateStores.read("subscriptions", { id }).then(async (subscription) => {
      ...
      // Cut off
      cutSubscription(subscription).catch((e) => log.debug("Cut failed:", e));
      // Update states
      const stateErrorHandler = (e) => {
        log.debug("Clean state error removeSubscription:", e?.message);
      };
      stateStores
        .setRemove("participants", { id: owner }, { subs: id })
        .catch(stateErrorHandler);
      stateStores
        .setRemove("nodes", { id: nodeId }, { subs: id })
        .catch(stateErrorHandler);
      // Notify controller
      const status = {
        type: "subscription",
  ...
  const updateSubscription = function (updates) {
    ...
    sub._updating = true;
    stateStores
      .read("subscriptions", { id: updates.id })
  ...
  (function initialize() {
    // Save service state
    stateStores
      .create("streamEngineNodes", { id: rpcID })
      ...
      ctrl.on("session-established", (id, session) => {
        ...
        if (session.source) {
          ...
          addPublication(session).catch((e) => {
            stopSession(session.id, session.type, "in", e?.message).catch(
              (e) => {
          ...
          addSubscription(session).catch((e) => {
            stopSession(session.id, session.type, "out", e?.message).catch(
              (e) => {
  ...
  that.onSessionSignaling = function (req, callback) {
    ...
    controllers[type]
      .onClientTransportSignaling(req.id, req.signaling)
      .then(() => {
  ...
  that.join = function (req, callback) {
    ...
    getDomainHandler(req.domain)
      .then(async (handler) => {
        ...
        const ppts = await stateStores.readMany("participants", {
          domain: req.domain,
        });
        const pubs = await stateStores.readMany("publications", {
          domain: req.domain,
        });
        response.room.participants = ppts.data.filter((ppt) => {
          return ppt.notifying;
        });
        response.room.streams = pubs.data
          .filter((plainPub) => {
  ...
  that.publish = function (req, callback) {
    ...
    if (controllers[type]) {
      stateStores
        .read("participants", { id: req.participant })
        .then(async (user) => {
          if (!user) {
