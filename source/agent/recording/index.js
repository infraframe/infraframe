...
module.exports = function (rpcClient, selfRpcId, parentRpcId, clusterWorkerIP) {
  ...
  var createFileIn = function (options, callback) {
    var avstream_options = { type: 'file', url: options.url };
  ...
  var createFileOut = function (connectionId, options) {
    var recording_path = path.join(
      global.config.recording.path,
      connectionId + '.' + options.connection.container
    );
    ...
    var connection = new AVStreamOut(avstream_options, function (error) {
      if (error) {
        ...
        notifyStatus(options.controller, connectionId, 'out', {
          type: 'ready',
          info: {
            host: selfRpcId.split('@')[1].split('_')[0],
            file: recording_path,
          },
        });
      }
    ...
    connection.addEventListener('fatal', function (error) {
      ...
      notifyStatus(options.controller, connectionId, 'out', {
        type: 'failed',
        reason: 'recording fatal error: ' + error,
      });
    });
  ...
  that.publish = function (connectionId, connectionType, options, callback) {
    ...
    router
      .addLocalSource(connectionId, connectionType, conn.source())
