...
describe("Test QuicTransportServer.", () => {
  ...
  it("Create send stream with unauthroized transport ID returns undefined.", (done) => {
  ...
  it("UUID convertions.", (done) => {
    const uuidStrings = [
      ...
      "d8bc24a9360b4320bf42704353d906db",
    ...
    for (let i = 0; i < uuidStrings.length; i++) {
      assert.deepEqual(
        uuidArrays[i],
        server._uuidStringToUint8Array(uuidStrings[i])
      );
      assert.deepEqual(
        uuidStrings[i],
        server._uuidBytesToString(uuidArrays[i])
      );
    }
    done();
  });
