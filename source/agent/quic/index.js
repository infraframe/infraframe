...
module.exports = function (rpcClient, selfRpcId, parentRpcId, clusterWorkerIP) {
  ...
  const getRpcPortal = async (roomId, participantId) => {
    ...
    for (let i = 0; i < retry; i++) {
      try {
        controller = await rpcChannel.makeRPC(controllerAgent.id, "getNode", [
    ...
    portal = await rpcChannel.makeRPC(controller, "getPortal", [participantId]);
    return portal;
  };
  ...
  const keystore = path.resolve(
    path.dirname(global.config.quic.keystorePath),
    cipher.kstore
  );
  cipher.unlock(cipher.k, keystore, (error, password) => {
    ...
    quicTransportServer = new QuicTransportServer(
      addon,
      global.config.quic.port,
      path.resolve(global.config.quic.keystorePath),
      password,
      validateToken
    );
    ...
    quicTransportServer.on("streamadded", (stream) => {
      log.debug(
        "A stream with session ID " + stream.contentSessionId + " is added."
      );
  ...
  const createFrameDestination = (subscriptionId, options, isDatagram) => {
    ...
    const frameDestination = new addon.WebTransportFrameDestination(
      subscriptionId,
      isDatagram
    );
    ...
    switch (connectionType) {
      case "quic":
        if (options.tracks && options.tracks.length) {
          ...
          const webTransportConnection = quicTransportServer.getConnection(
