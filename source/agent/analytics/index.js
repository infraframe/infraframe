...
module.exports = function (rpcClient, rpcId, agentId, clusterIp) {
  ...
  that.publish = function (connectionId, connectionType, options, callback) {
    ...
    if (connectionType !== "analytics") {
      return callback("callback", {
        type: "failed",
        reason: "invalid connctionType" + connectionType,
      });
    }
  ...
  that.subscribe = function (connectionId, connectionType, options, callback) {
    ...
    if (!algorithms[options.connection.algorithm]) {
      return callback("callback", {
        type: "failed",
        reason: "Not valid algorithm:" + options.connection.algorithm,
      });
    }
    ...
    if (!inputs[connectionId]) {
      ...
      callback(
        "callback",
        "error",
        "Connection already exist: " + connectionId
      );
    }
  };
      ...
      if (engine) {
          ...
          function onStats(stat) {
            if (stat === "disconnected") {
              ...
              callback(
                "callback",
                "error",
                "Failed in adding input to analytics."
              );
              return;
            }
          }
        ...
        if (engine.linkInput(conn)) {
          ...
          callback(
            "callback",
            "error",
            "Failed in linking input to analytics."
          );
        }
      }
    }
