...
const videoSwitch =
  require('../videoSwitch/build/Release/videoSwitch.node').VideoSwitch;
...
module.exports = function (rpcClient, selfRpcId, parentRpcId, clusterWorkerIP) {
  ...
  var notifyTransportStatus = function (controller, transportId, status) {
    rpcClient.remoteCast(controller, 'onTransportProgress', [
      transportId,
      status,
    ]);
  ...
  var notifyMediaUpdate = function (
    controller,
    publicTrackId,
    direction,
    mediaUpdate
  ) {
    rpcClient.remoteCast(controller, 'onMediaUpdate', [
      publicTrackId,
      direction,
      mediaUpdate,
    ]);
  ...
  var notifyTrackUpdate = function (controller, transportId, updateInfo) {
    rpcClient.remoteCast(controller, 'onTrackUpdate', [
      transportId,
      updateInfo,
    ]);
    // Emit GRPC notifications
  ...
  var handleTrackInfo = function (transportId, trackInfo, controller) {
    ...
    if (trackInfo.type === 'track-added') {
      ...
      if (track.direction === 'in') {
        ...
        router
          .addLocalSource(publicTrackId, 'webrtc', trackSource)
          .catch((e) => log.warn('Unexpected error during track add:', e));
      } else {
        router
          .addLocalDestination(publicTrackId, 'webrtc', track)
          .catch((e) => log.warn('Unexpected error during track add:', e));
      }
      ...
      router
        .removeConnection(publicTrackId)
        .then((ok) => {
  ...
  that.destroyTransport = function (transportId, callback) {
    ...
    if (!peerConnections.has(transportId)) {
      callback(
        'callback',
        'error',
        'Transport does not exists: ' + transportId
      );
      return;
    }
  ...
  that.publish = function (operationId, connectionType, options, callback) {
    ...
    if (mappingTransports.has(operationId)) {
      return callback('callback', {
        type: 'failed',
        reason: 'Connection already exists:' + operationId,
      });
    }
  ...
  that.onTransportSignaling = function (connectionId, msg, callback) {
    log.debug(
      'onTransportSignaling, connection id:',
      connectionId,
      'msg:',
      msg
    );
        ...
        (trackId) =>
          new Promise((resolve, reject) => {
            if (mediaTracks.has(trackId)) {
              ...
              mediaTracks
                .get(trackId)
                .onTrackControl('av', direction, action, resolve, reject);
            } else {
              resolve();
            }
          })
      ...
      for (let info of layers) {
        ...
        if (layerStream) {
          ...
          if (!router.hasConnection(layerStream.id)) {
            router
              .addLocalSource(layerStream.id, 'webrtc', layerStream.sender())
  ...
  that.createQualitySwitch = function (sourceStreams, callback) {
    ...
    for (const source of sourceStreams) {
      ...
      if (conn) {
        ...
        router.addLocalDestination(dispatcherId, 'dispatcher', dispatcher);
        router
          .linkup(dispatcherId, { video: { id: source.id } })
          .catch((e) => log.debug(e));
    ...
    router
      .addLocalSource(switchId, 'webrtc', vswitch)
