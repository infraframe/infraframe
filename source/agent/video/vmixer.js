...
class InputManager {
  ...
  add(streamId, codec, conn, avatar) {
    if (this.inputs[streamId]) return -1;
  ...
  enable(streamId, toPending) {
    if (!this.pendingInputs[streamId] || streamId === toPending) return -1;
...
function VMixer(rpcClient, clusterIP, VideoMixer, router, streamingEmitter) {
  ...
  var removeOutput = function (stream_id) {
    if (outputs[stream_id]) {
      ...
      for (var connectionId in output.connections) {
        output.dispatcher.removeDestination(
          'video',
          output.connections[connectionId].receiver()
        );
  ...
  that.initialize = function (
    ...
    var config = {
      ...
      backgroundcolor:
        typeof videoConfig.bgColor === 'string'
          ? colorMap[videoConfig.bgColor]
          : videoConfig.bgColor,
    ...
    layoutProcessor.on('layoutChange', function (layoutSolution) {
      ...
      if (typeof engine.updateLayoutSolution === 'function') {
        engine.updateLayoutSolution(
          layoutSolution.filter((obj) => {
    ...
    default_resolution = videoConfig.parameters.resolution || {
      width: 640,
      height: 480,
    };
  ...
  that.onFaultDetected = function (message) {
    if (message.purpose === 'conference' && controller) {
      if (
        (message.type === 'node' && message.id === controller) ||
        (message.type === 'worker' && controller.startsWith(message.id))
      ) {
        log.error(
          'Conference controller (type:',
          message.type,
          'id:',
          message.id,
          ') fault is detected, exit.'
        );
        that.deinit();
        process.exit();
      }
  ...
  that.generate = function (
    ...
    var bitrate_factor = isUnspecified(bitrate)
      ? 1.0
      : typeof bitrate === 'string'
      ? Number(bitrate.replace('x', ''))
      : 0;
    bitrate = bitrate_factor
      ? calcDefaultBitrate(codec, resolution, framerate, motion_factor) *
        bitrate_factor
      : bitrate;
    keyFrameInterval = isUnspecified(keyFrameInterval)
      ? default_kfi
      : keyFrameInterval;
    ...
    if (
      supported_codecs.encode.findIndex((c) => c.toLowerCase() === codec) < 0
    ) {
  ...
  that.publish = function (stream_id, stream_type, options, callback) {
    ...
    if (stream_type !== 'internal') {
      return callback(
        'callback',
        'error',
        'can not publish a stream to video engine through a non-internal connection'
      );
    }
    ...
    if (options.video === undefined || options.video.codec === undefined) {
      return callback(
        'callback',
        'error',
        'can not publish a stream to video engine without proper video codec'
      );
    }

    if (!inputManager.has(stream_id)) {
      log.debug(
        'publish 1, inputs.length:',
        inputManager.size(),
        'maxInputNum:',
        maxInputNum
      );
  ...
  that.subscribe = function (connectionId, connectionType, options, callback) {
    ...
    if (connectionType !== 'internal') {
      return callback(
        'callback',
        'error',
        'can not subscribe a stream from video engine through a non-internal connection'
      );
    }
    callback('callback', 'ok');
  };
  ...
  that.setRegion = function (stream_id, region_id, callback) {
    ...
    if (inputManager.has(stream_id)) {
      if (inputManager.isPending(stream_id)) {
        ...
        inputManager.enable(
          stream_id,
          inputManager.getStreamFromInput(originInput)
        );
