...
const QuicTransportStreamPipeline = require("./quicTransportStreamPipeline");
...
module.exports = function (rpcClient, selfRpcId, parentRpcId, clusterWorkerIP) {
  ...
  const incomingStreamPipelines = new Map(); // Key is publication ID, value is stream pipeline.
  const outgoingStreamPipelines = new Map(); // Key is subscription ID, value is stream pipeline.
  const frameSourceMap = new Map(); // Key is publication ID, value is WebTransportFrameSource.
  ...
  const clusterName =
    global && global.config && global.config.cluster
      ? global.config.cluster.name
      : undefined;
  var port =
    global && global.config && global.config.bridge
      ? global.config.bridge.port
      : 0;
  ...
  const createStreamPipeline = function (streamId, direction, options) {
    ...
    switch (connectionType) {
      case "mediabridge":
        if (options.originType === "webrtc") {
          log.info(
            "origin type is webrtc and pubArgs:",
            options.pubArgs,
            " and session info is:",
            clusters
          );
          var sessionid = clusters[dest].quicsession.getId();
          const pubs = options.pubArgs.map(
            (pubArg) =>
              new Promise((resolve, reject) => {
                ...
                var quicStream =
                  clusters[dest].quicsession.createBidirectionalStream();
          ...
          log.info(
            "Create quic stream to receive subscribed stream from remote cluster, clusters:",
            clusters
          );
  ...
  that.subscribe = function (connectionId, connectionType, options, callback) {
    ...
    router
      .addLocalDestination(connectionId, connectionType, conn)
  ...
  that.onFaultDetected = function (message) {
    ...
    if (message.purpose === "conference") {
      for (var conn_id in clusters) {
        if (
          (message.type === "node" &&
            message.id === clusters[conn_id].controller) ||
          (message.type === "worker" &&
            clusters[conn_id].controller.startsWith(message.id))
        ) {
          log.error(
            "Fault detected on controller (type:",
            message.type,
            "id:",
            message.id,
            ") of connection:",
  ...
  that.startCascading = function (data, callback) {
    ...
    if (!clusters[dest]) {
      return rpcReq
        .getController(clusterName, data.room)
        .then(function (controller) {
          log.info("Client get controller:", controller);
          var client = new addon.QuicTransportClient(
            data.mediaIP,
            data.mediaPort
          );
          ...
          client.onConnection(() => {
            ...
            log.info(
              "Create quic stream with controller:",
              controller,
              " and data:",
              data,
              " streamID:",
            ...
            incomingStream.onStreamData((msg) => {
              ...
              if (info.type === "track") {
                ...
                if (clusters[dest].controller) {
                  ...
                  rpcReq
                    .getController(clusterName, info.options.room)
                    .then(function (controller) {
                      clusters[dest].controller = controller;
                      log.info(
                        "client subscribe to controller:",
                        controller,
                        "connection id:",
                        connectionId,
                        " with info:",
                        info.options.media.tracks
                      );
          ...
          client.onConnectionFailed(() => {
            ...
            callback(
              "callback",
              "error",
              "Media bridge quic connection failed"
            );
          });
              ...
              if (clusters[dest].id === sessionId) {
                for (var item in clusters[dest].streams) {
                  rpcReq.unsubscribe(
                    controller,
                    "admin",
                    clusters[dest].streams[item].connid
                  );
                }
                delete clusters[dest];
              }
          ...
          client.onClosedStream((closedStreamId) => {
            ...
            if (
              clusters[client.dest] &&
              clusters[client.dest].streams[closedStreamId] &&
              clusters[client.dest].streams[closedStreamId].connid
            ) {
              rpcReq.unsubscribe(
                clusters[client.dest].controller,
                "admin",
                clusters[client.dest].streams[closedStreamId].connid
              );
  ...
  const subscribeFromLocal = (controller, info) => {
    ...
    rpcReq.getController(clusterName, info.room).then(function (controller) {
      log.info(
  ...
  const start = function () {
    const keystore = path.resolve(
      path.dirname(global.config.bridge.keystorePath),
      cipher.kstore
    );
    cipher.unlock(cipher.k, keystore, (error, password) => {
      ...
      server = new addon.QuicTransportServer(
        port,
        path.resolve(global.config.bridge.keystorePath),
        password
      );
      ...
      server.onNewSession((session) => {
        ...
        session.onNewStream((quicStream) => {
          ...
          quicStream.onStreamData((msg) => {
            ...
            if (info.type === "cluster") {
              rpcReq
                .getController(clusterName, info.room)
                .then(function (controller) {
              ...
              log.info(
                "Server stream id:",
                streamId,
                " get subscribe msg with subscribe info:",
                info,
                " and session:",
                sessionId
              );
        ...
        session.onClosedStream((closedStreamId) => {
          ...
          if (
            clusters[session.dest] &&
            clusters[session.dest].streams[closedStreamId] &&
            clusters[session.dest].streams[closedStreamId].connid
          ) {
            rpcReq.unsubscribe(
              clusters[session.dest].controller,
              "admin",
              clusters[session.dest].streams[closedStreamId].connid
            );
      ...
      server.onClosedSession((sessionId) => {
        ...
        for (var item in clusters[sessionId].streams) {
          rpcReq.unsubscribe(
            clusters[sessionId].controller,
            "admin",
            clusters[sessionId].streams[item].connid
          );
        }
