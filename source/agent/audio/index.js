...
module.exports = function (rpcClient, selfRpcId, parentRpcId, clusterWorkerIP) {
  ...
  var addInput = function (stream_id, owner, codec, options, on_ok, on_error) {
    if (engine) {
      ...
      if (engine.addInput(owner, stream_id, codec, conn)) {
        inputs[stream_id] = { owner: owner, connection: conn };
        log.debug(
          "addInput ok, for:",
          owner,
          "codec:",
  ...
  that.createInternalConnection = function (
    connectionId,
    direction,
    internalOpt,
    callback
    ...
    var portInfo = internalConnFactory.create(
      connectionId,
      direction,
      internalOpt
    );
  ...
  that.destroyInternalConnection = function (
    connectionId,
    direction,
    callback
  ) {
  ...
  that.publish = function (stream_id, stream_type, options, callback) {
    ...
    if (stream_type !== "internal") {
      return callback(
        "callback",
        "error",
        "can not publish a stream to audio engine through a non-internal connection"
      );
    }
    ...
    if (options.audio === undefined || options.audio.codec === undefined) {
      return callback(
        "callback",
        "error",
        "can not publish a stream to audio engine without proper audio codec"
      );
    }
  ...
  that.cutoff = function (connectionId) {
    ...
    if (connections[connectionId] && connections[connectionId].audioFrom) {
      if (outputs[connections[connectionId].audioFrom]) {
        outputs[
          connections[connectionId].audioFrom
        ].dispatcher.removeDestination(
          "audio",
          connections[connectionId].connection.receiver()
        );
      }
  ...
  that.init = function (
    ...
    if (service === "mixing") {
      ...
      callback(
        "callback",
        "error",
        "Unknown service type to init an audio node."
      );
    }
  };
