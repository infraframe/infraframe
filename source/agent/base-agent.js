...
class BaseAgent {
  ...
  createInternalConnection(connectionId, direction, internalOpt) {
    ...
    const portInfo = this.internalConnFactory.create(
      connectionId,
      direction,
      internalOpt
    );
  ...
  _setupConnection(connectionId, connectionType, direction, options) {
    ...
    return this.connections.addConnection(
      connectionId,
      connectionType,
      options.controller,
      conn,
  ...
  _destroyConnection(connectionId, direction) {
    ...
    return this.connections.removeConnection(connectionId).then(function (ok) {
  ...
  cleanup() {
    ...
    var cleans = connections.getIds().map((connId) => {
      ...
      return connections.removeConnection(connId).then(function (ok) {
        if (conn && conn.type === "internal") {
