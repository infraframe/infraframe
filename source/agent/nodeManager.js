...
module.exports = function (
  ...
  function cleanupNode(id) {
    processes[id].check_alive_interval &&
      clearInterval(processes[id].check_alive_interval);
  ...
  var launchNode = function () {
    ...
    child.on('message', function (message) {
      // currently only used for sending ready message from node to agent;
  ...
  var fillNodes = function () {
    ...
    var nodesToStart =
      spec.maxNodeNum < 0
        ? spaceInIdle
        : Math.min(spaceInIdle, spec.maxNodeNum - runningNodes);
  ...
  let findNodeUsedByRoom = (nodeList, roomId) => {
    for (let i in nodeList) {
      ...
      if (
        tasks[node_id] !== undefined &&
        tasks[node_id][roomId] !== undefined
      ) {
        return node_id;
      }
    }
  ...
  let pickInIdle = (room) => {
    ...
    setTimeout(() => {
      if (
        spec.maxNodeNum < 0 ||
        nodes.length + idle_nodes.length < spec.maxNodeNum
      ) {
        fillNodes();
  ...
  that.getNode = (task) => {
    ...
    return waitTillNodeReady(
      nodeId,
      1500 /*FIXME: Use a more reasonable timeout value instead of hard coding*/
    )
      .then((nodeId) => {
        addTask(nodeId, task);
  ...
  that.dropAllNodes = function (quietly) {
    ...
    Object.keys(processes).map(function (k) {
      !quietly &&
        onNodeAbnormallyQuit &&
        onNodeAbnormallyQuit(k, tasksOnNode(k));
      dropNode(k);
    });
  };
