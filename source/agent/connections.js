...
module.exports = function Connections() {
  ...
  var cutOffFrom = function (connectionId) {
    ...
    if (
      ...
      connections[connectionId].direction === "in"
    ) {
        ...
        if (connections[connection_id].direction === "out") {
          if (connections[connection_id].audioFrom === connectionId) {
            log.debug(
              ...
              connections[connection_id].audioFrom
            );
            var dest = connections[connection_id].connection.receiver("audio");
            if (dest) {
              const isNanObj = nanObjects.includes(dest.constructor.name)
                ? true
                : false;
              connections[connectionId].connection.removeDestination(
                "audio",
                dest,
                isNanObj
              );
          ...
          if (connections[connection_id].videoFrom === connectionId) {
            log.debug(
              "remove video subscription:",
              connections[connection_id].videoFrom
            );
  ...
  var cutOffTo = function (connectionId) {
    ...
    if (
      connections[connectionId] &&
      connections[connectionId].direction === "out"
    ) {
      ...
      if (
        audioFrom &&
        connections[audioFrom] &&
        connections[audioFrom].direction === "in"
      ) {
        ...
        const isNanObj = nanObjects.includes(dest.constructor.name)
          ? true
          : false;
        connections[audioFrom].connection.removeDestination(
          "audio",
          dest,
          isNanObj
        );
      ...
      if (
        videoFrom &&
        connections[videoFrom] &&
        connections[videoFrom].direction === "in"
      ) {
        ...
        const isNanObj = nanObjects.includes(dest.constructor.name)
          ? true
          : false;
        connections[videoFrom].connection.removeDestination(
          "video",
          dest,
          isNanObj
        );
  ...
  that.linkupConnection = function (
    connectionId,
    audioFrom,
    videoFrom,
    dataFrom
    ...
    for (const [name, from] of new Map([
      ["audio", audioFrom],
      ["video", videoFrom],
      ["data", dataFrom],
    ])) {
      if (from && !connections[from]) {
        ...
        return Promise.reject({
          type: "failed",
          reason: name + " stream does not exist:" + from,
        });
      }
      if (from) {
        ...
        if (!dest) {
          return Promise.reject({
            type: "failed",
            reason: "Destination connection(" + name + ") is not ready",
          });
        }
        const isNanObj = nanObjects.includes(dest.constructor.name)
          ? true
  ...
  that.onFaultDetected = function (message) {
    if (message.purpose === "conference") {
      for (var conn_id in connections) {
        if (
          (message.type === "node" &&
            message.id === connections[conn_id].controller) ||
          (message.type === "worker" &&
            connections[conn_id].controller.startsWith(message.id))
        ) {
          log.error(
