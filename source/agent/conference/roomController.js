...
module.exports.create = function (spec, on_init_ok, on_init_failed) {
  ...
  // Given mix stream ID, return the view
  ...
  var initView = function (view, viewSettings, onInitOk, onInitError) {
    ...
    var initMixer = (mixerId, type, mixConfig) =>
    ...
    initMixer(audio_mixer, 'amixer', viewSettings.audio).then(
      function onAudioReady(supportedAudio) {
        ...
        if (viewSettings.video) {
          ...
          mix_views[view].video = {
            mixer: null,
            supported_formats: { encode: [], decode: [] },
          };
          onInitOk();
        }
      },
  ...
  var initialize = function (on_ok, on_error) {
    ...
    if (config.views.length > 0) {
      ...
      Promise.all(viewProcessed)
        .then(function (results) {
          // Result for callback
          var viewCount = results.filter(function (re) {
            return re !== null;
          }).length;
  ...
  var newTerminal = function (
    ...
    if (terminals[terminal_id] === undefined) {
      ...
      var nodeLocality = preAssignedNode
        ? Promise.resolve(preAssignedNode)
        : rpcReq.getWorkerNode(
            cluster,
            purpose,
            { room: room_id, task: terminal_id },
            mediaPreference
          );

      return nodeLocality.then(
        function (locality) {
  ...
  var deleteTerminal = function (terminal_id) {
    ...
    if (terminals[terminal_id]) {
      if (
        ...
        rpcReq
          .recycleWorkerNode(
            terminals[terminal_id].locality.agent,
  ...
  var spreadStream = function (
    ...
    const on_spread_failed = function (reason) {
      ...
      const i = streams[stream_id]
        ? streams[stream_id].spread.findIndex((s) => {
    ...
    const on_spread_ok = function () {
      ...
      if (!streams[stream_id]) {
        on_error(`invalid stream_id: ${stream_id}`);
        return;
      }
  ...
  var shrinkStream = function (stream_id, target_node) {
    ...
    if (streams[stream_id] && terminals[streams[stream_id].owner]) {
      ...
      if (
        ...
        !isSpreadInUse(stream_id, target_node)
      ) {
        var i = streams[stream_id].spread.findIndex((s) => {
          return s.target === target_node;
        });
        if (i !== -1) {
          streams[stream_id].spread.splice(i, 1);
  ...
  var isSpreadInUse = function (stream_id, node) {
    var audio_subscribers =
      ...
      video_subscribers =
        ...
        [],
      subscribers = audio_subscribers.concat(
        video_subscribers.filter(function (item) {
          return audio_subscribers.indexOf(item) < 0;
        })
      );
    ...
    for (var i in subscribers) {
      if (
        ...
        terminals[subscribers[i]].locality.node === node
      ) {
        return true;
      }
    }
  ...
  var getMixedAudio = function (
    ...
    spawnMixedAudio(view, audio_format, subscriber, on_ok, on_error);
  };

  var getMixedVideo = function (
    view,
    format,
    resolution,
    framerate,
  ...
  var findExistingTranscodedAudio = function (
    axcoder,
    audio_format,
    on_ok,
    on_error
    ...
    for (var j in published) {
      if (
        streams[published[j]] &&
        streams[published[j]].audio &&
        streams[published[j]].audio.format === audio_format
      ) {
        on_ok(published[j]);
        return;
      }
  ...
  var spawnTranscodedVideo = function (
    vxcoder,
    format,
    resolution,
    framerate,
  ...
  var findExistingTranscodedVideo = function (
    vxcoder,
    format,
    resolution,
    framerate,
  ...
  var getTranscodedVideo = function (
    format,
    resolution,
    framerate,
    bitrate,
  ...
  var simulcastVideoMatched = function (
    stream_id,
    format,
    resolution,
    framerate,
      ...
      if (simulcastRid) {
        ...
        if (
          ...
          for (const rid in streams[stream_id].video.simulcast) {
            ...
            const combinedVideo = Object.assign(
              {},
              { format: streams[stream_id].video.format },
              simInfo
            );
    ...
    if (mixView) {
      ...
      if (streams[stream_id].video) {
        ...
        if (isSimulcastStream(stream_id)) {
          const matchedSimId = simulcastVideoMatched(
            stream_id,
            format,
            resolution,
            framerate,
  ...
  var unsubscribeStream = function (subscriber, subscription_id) {
    if (terminals[subscriber]) {
      ...
      if (audio_stream && streams[audio_stream]) {
        ...
        terminals[streams[audio_stream].owner] &&
          terminals[streams[audio_stream].owner].locality.node !== node &&
          shrinkStream(audio_stream, node);
        terminals[streams[audio_stream].owner] &&
          !isParticipantTerminal(streams[audio_stream].owner) &&
      ...
      if (video_stream && streams[video_stream]) {
        ...
        terminals[streams[video_stream].owner] &&
          terminals[streams[video_stream].owner].locality.node !== node &&
          shrinkStream(video_stream, node);
        terminals[streams[video_stream].owner] &&
          !isParticipantTerminal(streams[video_stream].owner) &&
        ...
        function () {
          streams[streamId] = {
            ...
            video: streamInfo.media.video
              ? {
                  format: formatStr(streamInfo.media.video),
  ...
  that.subscribe = function (
    ...
    if (
      (!mediaInfo.audio ||
        (streams[mediaInfo.audio.from] &&
          streams[mediaInfo.audio.from].audio) ||
        getViewOfMixStream(mediaInfo.audio.from)) &&
      (!mediaInfo.video ||
        (streams[mediaInfo.video.from] &&
          streams[mediaInfo.video.from].video) ||
        getViewOfMixStream(mediaInfo.video.from))
    ) {
      ...
      if (mediaInfo.audio) {
        ...
        audio_format = isMixStream
          ? getMixedFormat(
              mediaInfo.audio,
              mix_views[subView].audio.supported_formats
            )
          : getForwardFormat(
              mediaInfo.audio,
              streams[subAudioStream].audio.format,
      ...
      if (mediaInfo.video) {
        ...
        if (isMixStream) {
          // Is mix stream
          video_format = getMixedFormat(
            mediaInfo.video,
            mix_views[subView].video.supported_formats.encode
          );
        } else {
          // Is forward stream
          video_format = getForwardFormat(
            mediaInfo.video,
            streams[subVideoStream].video.format,
        ...
        mediaInfo.video &&
          ...
          (resolution = mediaInfo.video.parameters.resolution);
        mediaInfo.video &&
          mediaInfo.video.parameters &&
        ...
        mediaInfo.video &&
          mediaInfo.video.parameters &&
          mediaInfo.video.parameters.bitrate &&
          (bitrate = mediaInfo.video.parameters.bitrate);
        mediaInfo.video &&
          mediaInfo.video.parameters &&
      ...
      const createMapForSources = function (
        audioStreamId,
        videoStreamId,
        dataStreamId
      ) {
      ...
      var finally_ok = function (audioStream, videoStream, dataStream) {
        return function () {
          if (
            ...
            for (const [kind, streamId] of createMapForSources(
              audioStream,
              videoStream,
              dataStream
            )) {
              if (streamId) {
                streams[streamId][kind].subscribers =
                  streams[streamId][kind].subscribers || [];
                streams[streamId][kind].subscribers.push(terminal_id);
                terminals[terminal_id].subscribed[subscriptionId][kind] =
                  streamId;
              }
            }
            on_ok('ok');
  ...
  var allocateMediaProcessingNode = function (forWhom, usage) {
    return rpcReq.getWorkerNode(
      cluster,
      purpose,
      { room: room_id, task: terminal_id },
      mediaPreference /*FIXME: should take formats and usage specification into preference*/
    );
  };
          ...
          outputs.map(function (old_st) {
            ...
            return new Promise(function (resolve, reject) {
              getMixedVideo(
                view,
                old_st.video.format,
                old_st.video.resolution,
                old_st.video.framerate,
                    ...
                    .then(function () {
                      old_st.video.subscribers.forEach(function (t_id) {
                        if (terminals[t_id]) {
                          for (var sub_id in terminals[t_id].subscribed) {
                            if (
                              terminals[t_id].subscribed[sub_id].video ===
                              old_st.old_stream_id
                            ) {
                              const fromInfo = {
                                video: getStreamLinkInfo(stream_id),
                              };
                              makeRPC(
                                rpcClient,
                                terminals[t_id].locality.node,
                                ...
                                function () {
                                  streams[stream_id].video.subscribers =
                                    streams[stream_id].video.subscribers || [];
                                  streams[stream_id].video.subscribers.push(
                                    t_id
                                  );
                                  terminals[t_id].subscribed[sub_id].video =
                                    stream_id;
                    ...
                    .then(function () {
                      old_st.audio.subscribers.forEach(function (t_id) {
                        if (terminals[t_id]) {
                          for (var sub_id in terminals[t_id].subscribed) {
                            if (
                              terminals[t_id].subscribed[sub_id].audio ===
                              old_st.old_stream_id
                            ) {
                              const fromInfo = {
                                audio: getStreamLinkInfo(stream_id),
                              };
                              makeRPC(
                                rpcClient,
                                terminals[t_id].locality.node,
                                ...
                                function () {
                                  streams[stream_id].audio.subscribers =
                                    streams[stream_id].audio.subscribers || [];
                                  streams[stream_id].audio.subscribers.push(
                                    t_id
                                  );
                                  terminals[t_id].subscribed[sub_id].audio =
                                    stream_id;
  ...
  that.getViewCapability = function (view) {
    if (mix_views[view]) {
      return {
        ...
        video: {
          encode:
            mix_views[view].video.supported_formats.encode.map(
              video_format_obj
            ),
          decode:
            mix_views[view].video.supported_formats.decode.map(
              video_format_obj
            ),
        },
      };
