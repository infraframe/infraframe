...
module.exports.create = function (
  ...
  const terminateSession = (sessionId) => {
    ...
    if (
      ...
      return rpcReq
        .terminate(session.locality.node, sessionId, session.direction)
        .then(function () {
          log.debug(
            "to recycleWorkerNode:",
            session.locality,
            "task:",
            sessionId
          );
          return rpcReq.recycleWorkerNode(
            session.locality.agent,
            session.locality.node,
            { room: in_room, task: sessionId }
          );
        })
        .then(function () {
  ...
  const onReady = (sessionId, status) => {
    ...
    if (
      session.options.type === "webrtc" ||
      session.options.originType === "webrtc"
    ) {
    ...
    session.state = "connected";
    var media = {},
      info = { type: session.options.type, owner: session.owner };

    if (session.direction === "in") {
      session.options.media.audio &&
        (media.audio = !!audio ? audio || {} : false);
      media.audio &&
        session.options.media.audio &&
        session.options.media.audio.source &&
        (media.audio.source = session.options.media.audio.source);
      session.options.media.video &&
        (media.video = !!video ? video || {} : false);
      media.video &&
        session.options.media.video &&
        session.options.media.video.source &&
        (media.video.source = session.options.media.video.source);
      media.video &&
        session.options.media.video &&
        session.options.media.video.parameters &&
        session.options.media.video.parameters.resolution &&
        (media.video.resolution =
          session.options.media.video.parameters.resolution);
      media.video &&
        session.options.media.video &&
        session.options.media.video.parameters &&
        session.options.media.video.parameters.framerate &&
        (media.video.framerate =
          session.options.media.video.parameters.framerate);

      simulcast && simulcast.video && (media.video.simulcast = true);
      session.options.attributes &&
        (info.attributes = session.options.attributes);
    } else {
      if (session.options.media.audio) {
        media.audio = { from: session.options.media.audio.from };
        session.options.media.audio.format &&
          (media.audio.format = session.options.media.audio.format);
      ...
      if (session.options.media.video) {
        ...
        session.options.media.video.format &&
          (media.video.format = session.options.media.video.format);
        session.options.type === "webrtc" && (media.video.format = video);
        session.options.media.video.parameters &&
          (media.video.parameters = session.options.media.video.parameters);
        session.options.media.video.simulcastRid &&
          (media.video.simulcastRid = session.options.media.video.simulcastRid);
      }
      ...
      if (
        session.options.type === "recording" ||
        session.options.originType === "recording"
      ) {
        info.location = status.info;
      }

      if (
        session.options.type === "streaming" ||
        session.options.originType === "streaming"
      ) {
        info.url = status.info;
      }

      if (
        session.options.type === "analytics" ||
        session.options.originType === "analytics"
      ) {
  ...
  that.onSessionSignaling = (sessionId, signaling) => {
    ...
    return rpcReq
      .onSessionSignaling(
        sessions[sessionId].locality.node,
        sessionId,
        signaling
      )
      .catch((e) => {
  ...
  that.participantLeave = function (participantId) {
    ...
    for (var session_id in sessions) {
      if (
        sessions[session_id].owner === participantId &&
        !sessions[session_id].locality.agent.startsWith("mediabridge")
      ) {
  ...
  that.initiate = function (
    ...
    if (sessionOptions.type === "webrtc") {
      return rtc_controller.initiate(
        participantId,
        sessionId,
        direction,
        origin,
    ...
    if (sessionOptions.type === "quic") {
      return quic_controller.initiate(
        participantId,
        sessionId,
        direction,
        origin,
    ...
    return rpcReq
      ...
      .then(function (accessLocality) {
        ...
        log.debug(
          "getWorkerNode ok, participantId:",
          participantId,
          "sessionId:",
          sessionId,
          "locality:",
        ...
        if (sessions[sessionId] === undefined) {
          ...
          rpcReq
            .recycleWorkerNode(locality.agent, locality.node, {
              room: in_room,
              task: sessionId,
            })
            .catch(function (reason) {
        ...
        sessionOptions.connection &&
          (options.connection = sessionOptions.connection);
        sessionOptions.transport &&
          (options.transport = sessionOptions.transport);
      ...
      .then(
        function () {
          ...
          if (sessions[sessionId] === undefined) {
            log.debug("Session has been aborted, sessionId:", sessionId);
            rpcReq
              .terminate(locality.node, sessionId, direction)
              .catch(function (reason) {
    ...
    return rpcReq
      ...
      .then(function (accessLocality) {
        ...
        session.options.connection &&
          (options.connection = session.options.connection);
      ...
      .then(
        function () {
          ...
          if (sessions[sessionId] === undefined) {
            log.debug("Session has been aborted, sessionId:", sessionId);
            rpcReq
              .terminate(locality.node, sessionId, session.direction)
              .catch(function (reason) {
                log.debug("Terminate fail:", reason);
              });
            rpcReq
    ...
    for (var session_id in sessions) {
      ...
      if (locality && isImpacted(locality, faultType, faultId)) {
        ...
        on_session_aborted(
          ...
          session_id,
          direction,
          "Access node exited unexpectedly"
        );
