...
describe("rpcRequest.getWorkerNode", function () {
  it("Should succeed if scheduling agent and requiring node succeed.", function () {
    ...
    mockRpcChannel.makeRPC
      .onCall(0)
      .returns(
        Promise.resolve({
          id: "rpcIdOfWorkerAgent",
          info: { ip: "x.x.x.x", purpose: "purpose", state: 2, max_load: 0.85 },
        })
      );
    mockRpcChannel.makeRPC
      .onCall(1)
      .returns(Promise.resolve("rpcIdOfWorkerNode"));
    ...
    return req
      .getWorkerNode(
        "owt-cluster",
        "purpose",
        { room: "Session", task: "sessionId" },
        { isp: "isp", region: "region" }
      )
      .then(function (result) {
        expect(result).to.deep.equal({
          agent: "rpcIdOfWorkerAgent",
          node: "rpcIdOfWorkerNode",
        });
        expect(mockRpcChannel.makeRPC.getCall(0).args).to.deep.equal([
          "owt-cluster",
          "schedule",
          ["purpose", "sessionId", { isp: "isp", region: "region" }, 30000],
        ]);
        expect(mockRpcChannel.makeRPC.getCall(1).args).to.deep.equal([
          "rpcIdOfWorkerAgent",
          "getNode",
          [{ room: "Session", task: "sessionId" }],
        ]);
  ...
  it("Should fail if scheduling agent timeout or error occurs.", function () {
    ...
    mockRpcChannel.makeRPC
      .onCall(0)
      .returns(Promise.reject("timeout or error while getting agent"));

    var req = rpcRequest(mockRpcChannel);

    return expect(
      req.getWorkerNode("owt-cluster", "purpose", {
        room: "Session",
        task: "sessionId",
      })
    ).to.be.rejectedWith("timeout or error while getting agent");
  });
  ...
  it("Should fail if requiring node timeout or error occurs.", function () {
    ...
    mockRpcChannel.makeRPC
      .onCall(0)
      .returns(Promise.resolve("rpcIdOfWorkerAgent"));
    mockRpcChannel.makeRPC
      .onCall(1)
      .returns(Promise.reject("timeout or error"));
...
describe("rpcRequest.initiate", function () {
  it("Should succeed if rpcChannel.makeRPC succeeds.", function () {
    ...
    return req
      .initiate("rpcIdOfController", "sessionId", "webrtc", "in", {
        media: "mediaOptionsObj",
        connection: "connectionOptionsObj",
      })
      .then(function (result) {
        expect(result).to.deep.equal("ok");
        expect(mockRpcChannel.makeRPC.getCall(0).args[0]).to.equal(
          "rpcIdOfController"
        );
        expect(mockRpcChannel.makeRPC.getCall(0).args[1]).to.equal("publish");
        expect(mockRpcChannel.makeRPC.getCall(0).args[2][0]).to.equal(
          "sessionId"
        );
        expect(mockRpcChannel.makeRPC.getCall(0).args[2][1]).to.equal("webrtc");
        expect(mockRpcChannel.makeRPC.getCall(0).args[2][2]).to.deep.equal({
          media: "mediaOptionsObj",
          connection: "connectionOptionsObj",
        });
        return req
          .initiate("rpcIdOfController", "sessionId", "webrtc", "out", {
            media: "mediaOptionsObj",
            connection: "connectionOptionsObj",
          })
          .then(function (result) {
            expect(result).to.deep.equal("ok");
            expect(mockRpcChannel.makeRPC.getCall(1).args[0]).to.equal(
              "rpcIdOfController"
            );
            expect(mockRpcChannel.makeRPC.getCall(1).args[1]).to.equal(
              "subscribe"
            );
            expect(mockRpcChannel.makeRPC.getCall(1).args[2][0]).to.equal(
              "sessionId"
            );
            expect(mockRpcChannel.makeRPC.getCall(1).args[2][1]).to.equal(
              "webrtc"
            );
            expect(mockRpcChannel.makeRPC.getCall(1).args[2][2]).to.deep.equal({
              media: "mediaOptionsObj",
              connection: "connectionOptionsObj",
            });
          });
  ...
  it("Should fail if rpcChannel.makeRPC timeout or error occurs.", function () {
    ...
    return Promise.all([
      expect(
        req.initiate("rpcIdOfController", "sessionId", "webrtc", "in", {
          media: "mediaOptionsObj",
          connection: "connectionOptionsObj",
        })
      ).to.be.rejectedWith("timeout or error"),
      expect(
        req.initiate("rpcIdOfController", "sessionId", "webrtc", "out", {
          media: "mediaOptionsObj",
          connection: "connectionOptionsObj",
        })
      ).to.be.rejectedWith("timeout or error"),
...
describe("rpcRequest.terminate", function () {
  it("Should succeed if rpcChannel.makeRPC succeeds.", function () {
    ...
    expect(req.terminate("rpcIdOfWorkerNode", "sessionId", "out")).to.become(
      "ok"
    );
    expect(req.terminate("rpcIdOfWorkerNode", "sessionId", "in")).to.become(
      "ok"
    );
    expect(
      req.recycleWorkerNode("rpcIdOfWorkerAgent", "rpcIdOfWorkerNode", {
        room: "Session",
        task: "sessionId",
      })
    ).to.become("ok");
...
describe("rpcRequest.getRoomConfig/onSessionSignaling/mediaOnOff/sendMsg/broadcast/dropUser", function () {
  it("Should succeed if rpcChannel.makeRPC succeeds.", function () {
    ...
    var onSessionSignaling = req.onSessionSignaling(
      "rpcIdOfWorkerNode",
      "sessionId",
      "soacObj"
    );
