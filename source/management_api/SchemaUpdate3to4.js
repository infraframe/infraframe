...
function fracString(num) {
  ...
  try {
    if (
      typeof num.denominator === "number" &&
      typeof num.numerator === "number"
    ) {
...
function upgradeRoom(oldConfig) {
  ...
  for (oldName in oldConfig.views) {
    ...
    newView.video.parameters.resolution = upgradeResolution(
      oldMedia.video.resolution
    );
    ...
    newView.video.layout.templates.custom = upgradeLayoutCustom(
      oldMedia.video.layout.custom
    );
...
function processRoom(rooms, i, onFinishRoom) {
  if (i >= rooms.length) {
    if (typeof onFinishRoom === "function") onFinishRoom();
    return;
  }
...
function processServices(services, i, onFinishService) {
  if (i >= services.length) {
    if (typeof onFinishService === "function") onFinishService();
    return;
  }
...
function main(next) {
  ...
  rl.question(
    "This operation will overwrite the old database, which " +
      ...
      "to proceed this operation anyway?[y/n]",
    (answer) => {
      rl.close();
