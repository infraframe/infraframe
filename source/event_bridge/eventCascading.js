...
var EventCascading = function (spec, rpcReq) {
  ...
  that.notify = function (participantId, msg, data) {
    if (controllers[msg.rpcId]) {
      if (msg.type === "initialize") {
        log.info(
          "Send initialize data to session:",
          msg.session,
          " and data:",
          data
        );
        if (sessions[msg.session].streams[msg.stream]) {
          sessions[msg.session].streams[msg.stream].quicstream.send(
            JSON.stringify(data)
          );
        } else {
  ...
  that.startCascading = function (data, on_ok, on_error) {
    ...
    if (!sessions[clientID]) {
      return rpcReq
        .getController(cluster_name, data.room)
        .then(function (controller) {
          ...
          client.onClosedStream((closedStreamId) => {
            ...
            if (
              sessions[client.dest] &&
              sessions[client.dest].streams[closedStreamId]
            ) {
              var event = {
                type: "onCascadingDisconnected",
              };
              rpcReq.handleCascadingEvents(
                sessions[client.dest].streams[closedStreamId].controller,
                self_rpc_id,
                sessions[client.dest].target,
                event
              );
            }
          });
          ...
          client.onConnectionClosed((sessionId) => {
            ...
            for (var item in sessions[client.dest].streams) {
              ...
              rpcReq.handleCascadingEvents(
                sessions[client.dest].streams[item].controller,
                self_rpc_id,
                sessions[client.dest].target,
                event
              );
      ...
      if (
        sessions[clientID].cascadedRooms &&
        sessions[clientID].cascadedRooms[data.room]
      ) {
  ...
  that.start = function () {
    ...
    cipher.unlock(cipher.k, keystore, (error, password) => {
      ...
      server = new quicCas.QuicTransportServer(
        port,
      ...
      server.onNewSession((session) => {
        ...
        session.onNewStream((quicStream) => {
          ...
          quicStream.onStreamData((msg) => {
            ...
            if (event.type === "bridge") {
              rpcReq
                .getController(cluster_name, event.room)
                .then(function (controller) {
                ...
                .then(function (token) {
                  ...
                  if (session.checkToken) {
                    var controller =
                      sessions[sessionId].streams[streamId].controller;
        ...
        session.onClosedStream((closedStreamId) => {
          ...
          if (
            sessions[sessionId] &&
            sessions[session.id].streams[closedStreamId]
          ) {
            delete controllers[
              sessions[session.id].streams[closedStreamId].controller
            ][session.dest];
            var event = {
              type: "onCascadingDisconnected",
            };
      ...
      server.onClosedSession((sessionId) => {
        ...
        if (sessions[sessionId].streams) {
          for (var item in sessions[sessionId].streams) {
            ...
            rpcReq.handleCascadingEvents(
              sessions[sessionId].streams[item].controller,
              self_rpc_id,
              sessions[sessionId].target,
              event
            );
    ...
    return new Promise((resolve, reject) => {
      for (var item in sessions) {
        ...
        for (var id in sessions[item].streams) {
          rpcReq.handleCascadingEvents(
            sessions[item].streams[id].controller,
            self_rpc_id,
            sessions[item].target,
            event
          );
        }
      }
    ...
    return new Promise((resolve, reject) => {
      for (var item in sessions) {
        for (var id in sessions[item].streams) {
          if (
            (message.type === "node" &&
              message.id === sessions[item].streams[id].controller) ||
            (message.type === "worker" &&
              sessions[item].streams[id].controller.startsWith(message.id))
          ) {
            log.error(
