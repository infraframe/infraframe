...
#ifdef ENABLE_MSDK
boost::shared_ptr<owt_base::MsdkFrame> FrameProcesser::getMsdkFrame(const uint32_t width, const uint32_t height)
{
    int32_t nullIndex = -1;
    int32_t freeIndex = -1;
...
void FrameProcesser::onFrame(const Frame& frame)
{
    ...
    if (m_format == FRAME_FORMAT_MSDK) {
        boost::shared_ptr<owt_base::MsdkFrame> msdkFrame = getMsdkFrame(width, height);
        if (!msdkFrame) {
...
void FrameProcesser::SendFrame(boost::shared_ptr<owt_base::MsdkFrame> msdkFrame, uint32_t timeStamp)
{
    ...
    outFrame.format = owt_base::FRAME_FORMAT_MSDK;
...
#ifdef ENABLE_MSDK
    if (m_format == FRAME_FORMAT_MSDK) {
        boost::shared_ptr<owt_base::MsdkFrame> msdkFrame;
        {
            boost::shared_lock<boost::shared_mutex> lock(m_mutex);
