...
bool MsdkBase::init()
{
    ...
    if (dec) {
        ...
        for (auto& codec : codecs) {
            ...
            if (sts == MFX_ERR_NONE) {
                ELOG_INFO("Supported decoder: %c%c%c%c", codec & 0xff, (codec >> 8) & 0xff, (codec >> 16) & 0xff, (codec >> 24) & 0xff);
    ...
    if (enc) {
        ...
        for (auto& codec : codecs) {
            ...
            if (sts == MFX_ERR_NONE) {
                ELOG_INFO("Supported encoder: %c%c%c%c", codec & 0xff, (codec >> 8) & 0xff, (codec >> 16) & 0xff, (codec >> 24) & 0xff);
...
void MsdkBase::printfVideoParam(mfxVideoParam* pVideoParam, DumpType type)
{
    ...
    printf("%s - %s++++++++++\n", __FUNCTION__, type == MFX_DEC ? "DECODE" : (type == MFX_VPP) ? "VPP"
                                                                                               : "ENCODE");
