...
static bool testEncoderDecoderPipeline(const string& encoderDecoderPipeline)
{
    ...
    string pipelineStr = "videotestsrc num-buffers=1 ! "
                         "capsfilter "
                         "caps=video/x-raw,format=(string)I420,width=(int)256,height=(int)256 ! "
        + encoderDecoderPipeline + " ! appsink name=sink";

    gst::unique_ptr<GError> error;
    auto pipeline = gst::unique_from_ptr(
        GST_PIPELINE(gst_parse_launch(pipelineStr.c_str(), out_ptr(error))));
    if (error) {
        return false;
    }

    auto sink = gst::unique_from_ptr(
    ...
    if (gst_element_get_state(
            GST_ELEMENT(pipeline.get()), &state, nullptr, Timeout)
    ...
    auto sample = gst::unique_from_ptr(
        gst_app_sink_try_pull_sample(GST_APP_SINK(sink.get()), Timeout));
