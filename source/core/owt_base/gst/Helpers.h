...
namespace gst {
namespace internal {
    // Explicit instanciation of the template is done in the following section for
    // any Gst type that we wish to delete Everything else won't compile, so that we
    // can catch undefined deleters and add them to this list This templated
    // function is then used by a class gst::gst_delete, which is implemented based
    // on the standard std::default_delete
    template <typename T>
    ...
    template <>
    inline void gst_deleter<gchar>(gchar* element) { g_free(element); }
    template <>
    ...
    // bitmasks/flags
    // Overloads of operator| are provided outside in the global namespace,
    // forwarding computation to this implementation
    template <typename T>
...
// Easily create a std::shared_ptr from a raw pointer using TAD, using a
// gst::gst_delete deleter
template <typename T>
...
// RAII wrapper for request pads, which need a reference to the element from
// which they come for cleanup
class GstRequestPad {
public:
