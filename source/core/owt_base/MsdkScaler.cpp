...
DEFINE_LOGGER(MsdkScaler, "owt.MsdkScaler");
...
{
    ELOG_DEBUG("%dx%d(%d-%d-%d-%d) -> %dx%d(%d-%d-%d-%d)", inWidth, inHeight, inCropX, inCropY, inCropW, inCropH, outWidth, outHeight, outCropX, outCropY, outCropW, outCropH);
    ...
    int outWidth, int outHeight, int outCropX, int outCropY, int outCropW, int outCropH)
{
    ...
    if (m_vppParam->vpp.In.Width != inWidth
        ...
        || m_vppParam->vpp.Out.CropW != outCropW
        || m_vppParam->vpp.Out.CropH != outCropH) {
        updateVppParam(inWidth, inHeight, inCropX, inCropY, inCropW, inCropH, outWidth, outHeight, outCropX, outCropY, outCropW, outCropH);
...
retry:
    ...
    if (sts == MFX_WRN_DEVICE_BUSY) {
        ...
        ELOG_ERROR("src(%p), %dx%d(%d-%d-%d-%d), locked %d", srcMsdkFrame, srcMsdkFrame->getWidth(), srcMsdkFrame->getHeight(), srcMsdkFrame->getCropX(), srcMsdkFrame->getCropY(), srcMsdkFrame->getCropW(), srcMsdkFrame->getCropH(), srcMsdkFrame->getLockedCount());
        ELOG_ERROR("dst(%p), %dx%d(%d-%d-%d-%d), locked %d", dstMsdkFrame, dstMsdkFrame->getWidth(), dstMsdkFrame->getHeight(), dstMsdkFrame->getCropX(), dstMsdkFrame->getCropY(), dstMsdkFrame->getCropW(), dstMsdkFrame->getCropH(), dstMsdkFrame->getLockedCount());
...
{
    if (!setupVpp(srcMsdkFrame->getWidth(), srcMsdkFrame->getHeight(), srcMsdkFrame->getCropX(), srcMsdkFrame->getCropY(), srcMsdkFrame->getCropW(), srcMsdkFrame->getCropH(), dstMsdkFrame->getWidth(), dstMsdkFrame->getHeight(), dstMsdkFrame->getCropX(), dstMsdkFrame->getCropY(), dstMsdkFrame->getCropW(), dstMsdkFrame->getCropH())) {
        return false;
    }
...
bool MsdkScaler::convert(MsdkFrame* srcMsdkFrame, int srcCropX, int srcCropY, int srcCropW, int srcCropH, MsdkFrame* dstMsdkFrame, int dstCropX, int dstCropY, int dstCropW, int dstCropH)
{
    if (!setupVpp(srcMsdkFrame->getWidth(), srcMsdkFrame->getHeight(), srcCropX, srcCropY, srcCropW, srcCropH, dstMsdkFrame->getWidth(), dstMsdkFrame->getHeight(), dstCropX, dstCropY, dstCropW, dstCropH)) {
        return false;
    }
