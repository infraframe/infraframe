...
class RankRecorder : public infraframe::AudioRanker::Visitor {
public:
    void onRankChange(
...
class TestSource : public infraframe::FrameSource {
public:
    void generateFrame(const infraframe::Frame& frame)
    {
        deliverFrame(frame);
    }
};

class TestDestination : public infraframe::FrameDestination {
public:
    TestDestination()
        : m_count(0)
    {
...
{
    infraframe::AudioRanker ranker(&recorder, false, 10);
...
BOOST_AUTO_TEST_CASE(TwoOutputs)
{
    ...
    infraframe::AudioRanker ranker(&recorder, false, 0);
    ranker.addOutput(&dest1);
