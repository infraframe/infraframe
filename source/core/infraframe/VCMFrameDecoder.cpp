...
#include "gst//H264VideoDecoders.h"
...
DEFINE_LOGGER(VCMFrameDecoder, "infraframe.VCMFrameDecoder");
...
bool VCMFrameDecoder::init(FrameFormat format)
{
    ...
    ELOG_DEBUG_T("Create decoder(%s)", getFormatStr(format));
    switch (format) {
    case FRAME_FORMAT_H264:
        codecType = VideoCodecType::kVideoCodecH264;
        m_decoder.reset(new NVH264GStreamerVideoDecoder());
        break;
    // case FRAME_FORMAT_H265:
    //     codecType = VideoCodecType::kVideoCodecH265;
    //     m_decoder.reset(GStreamerFrameDecoderFactory::Create(cricket::VideoCodec(cricket::kH265CodecName)));
    //     break;
    default:
        ELOG_ERROR_T("Unsupported video frame format %d(%s)", format, getFormatStr(format));
        return false;
    }
    ...
    // https://groups.google.com/g/discuss-webrtc/c/qrTGrptFFfk
    // https://issues.webrtc.org/issues/42223225
    if (m_decoder->InitDecode(&codecSettings, webrtc::CpuInfo::DetectNumberOfCores()) != WEBRTC_VIDEO_CODEC_OK) {
        ELOG_ERROR_T("Video decoder init failed.");
        return false;
    }
...
void VCMFrameDecoder::onFrame(const Frame& frame)
{
    ...
    ELOG_DEBUG_T("onFrame(%s), %s, %dx%d, length(%d)",
    ...
    image._encodedWidth = frame.additionalInfo.video.width;
    image._encodedHeight = frame.additionalInfo.video.height;
