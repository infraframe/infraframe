...
public:
    EncodeOut(int32_t streamId, infraframe::VideoFrameEncoder* owner, infraframe::FrameDestination* dest)
        : m_streamId(streamId)
        , m_owner(owner)
        , m_out(dest)
    {
    ...
    void onFeedback(const infraframe::FeedbackMsg& msg)
    {
        if (msg.type == infraframe::VIDEO_FEEDBACK) {
    ...
    void onEncoded(const infraframe::Frame& frame)
    {
        deliverFrame(frame);
    }
...
private:
    int32_t m_streamId;
    infraframe::VideoFrameEncoder* m_owner;
    infraframe::FrameDestination* m_out;
};
