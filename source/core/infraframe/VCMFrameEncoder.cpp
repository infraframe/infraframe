...
DEFINE_LOGGER(VCMFrameEncoder, "infraframe.VCMFrameEncoder");
...
VCMFrameEncoder::VCMFrameEncoder(FrameFormat format, VideoCodecProfile profile, bool useSimulcast)
    ...
    , m_bsDumpfp(nullptr)
{
    m_bufferManager.reset(new I420BufferManager(10));
...
int32_t VCMFrameEncoder::generateStream(uint32_t width, uint32_t height, uint32_t frameRate, uint32_t bitrateKbps, uint32_t keyFrameIntervalSeconds, infraframe::FrameDestination* dest)
{
...
#endif
    {
        ...
        case FRAME_FORMAT_H264:
            m_encoder.reset(new NVH264GStreamerVideoEncoder({}));
        ...
        //     m_encoder.reset(H265Encoder::Create(cricket::VideoCodec(cricket::kH265CodecName)));
        //     VCMCodecDataBase::Codec(kVideoCodecH265, &codecSettings);
        //     codecSettings.H265()->frameDroppingOn = true;

        //     codecSettings.H265()->keyFrameInterval = frameRate * keyFrameIntervalSeconds;
        //     break;
        default:
    ...
    if (ret) {
        ELOG_ERROR_T("Video encoder init failed.\n");
        return -1;
    }
...
void VCMFrameEncoder::requestKeyFrame(int32_t streamId)
{
    ...
    auto it = m_streams.find(streamId);
    if (it != m_streams.end()) {
        ELOG_DEBUG_T("Found stream: [%d]", streamId);
...
{
    boost::shared_lock<boost::shared_mutex> lock(m_mutex);
    ELOG_DEBUG("VCMFrameEncoder::onFrame");
    if (m_streams.size() == 0) {
        ELOG_DEBUG("Streams == 0");
        return;
    }

    boost::shared_ptr<webrtc::VideoFrame> videoFrame = frameConvert(frame);
    if (videoFrame == nullptr) {
        ELOG_DEBUG("videoFrame == nullptr");
        return;
    }
...
{
    boost::shared_lock<boost::shared_mutex> lock(m_mutex);
    ELOG_DEBUG("VCMFrameEncoder::encode: %p", this);
    int ret;
    ...
    ELOG_DEBUG("m_encoder->Encode [BEGIN]");
    ret = m_encoder->Encode(*frame.get(), nullptr, types.size() ? &types : nullptr);
    ELOG_DEBUG("m_encoder->Encode [END] [%d]", ret);
    if (ret != 0) {
    ...
    const RTPFragmentationHeader* fragmentation)
{
    ...
    if (!m_streams.empty()) {
        ...
        ELOG_DEBUG("SendData, %s, %dx%d, %s, length(%d), timestamp %d",
