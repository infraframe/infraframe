  ...
  var getSipAgent = function (for_whom, on_ok, on_failed) {
    ...
    var tryFetchingAgent = function (attempt) {
      ...
      makeRPC(
        ...
        [
          "sip",
          for_whom /*FIXME: use room_id as taskId temporarily, should use for_whom instead later.*/,
          "preference" /*FIXME: should fill-in actual preference*/,
          10 * 1000,
        ],
        function (result) {
          on_ok(result);
        ...
        function (reason) {
          if (keepTrying) {
            ...
            setTimeout(function () {
              tryFetchingAgent(
                attempt -
                  (typeof reason === "string" && reason.startsWith("Timeout")
                    ? 6
                    : 1)
              );
            }, 200);
