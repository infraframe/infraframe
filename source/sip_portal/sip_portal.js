...
var rebuildErizo = function (erizo_id) {
  ...
  for (var roomId in erizos) {
    if (erizos[roomId] === erizo_id) {
      // Get failed room id
      log.info(
        "SIP agent failed, try to re-assign an agent work for room:",
        roomId
      );
...
function initSipRooms() {
  ...
  dataAccess.room.sips().then(function (rooms) {
    // Get sip rooms here
...
function createSipConnectivity(room_id, sip_server, sip_user, sip_passwd) {
  ...
  roomPromises[room_id] = roomPromises[room_id].then(() => {
    ...
    return new Promise((resolve, reject) => {
      helper.allocateSipErizo(
        { room: room_id, task: room_id },
        function (erizo) {
          log.debug("allocateSipErizo", erizo);
          helper.initSipErizo(
            erizo.id,
            room_id,
            { sip_server, sip_user, sip_passwd },
            function (result) {
...
function startup() {
  ...
  if (enableGrpc) {
    ...
    grpcTools
      .startServer("sipPortal", grpcApi, grpcPort)
      .then((port) => {
  ...
  amqper.connect(
      ...
      amqper.asRpcClient(
        function (rpcClnt) {
          ...
          amqper.asRpcServer(
            ...
            function (reason) {
              log.error(
                "initializing sip-portal as rpc server failed, reason:",
                reason
              );
              process.exit();
            }
    ...
    function (reason) {
      log.error(
        "Sip-portal connect to rabbitMQ server failed, reason:",
        reason
      );
      process.exit();
    }
