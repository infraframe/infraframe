          ...
          reply_q.subscribe(function (message) {
            try {
              ...
              if (call_map[message.corrID] !== undefined) {
                ...
                call_map[message.corrID].fn[message.type].call(
                  {},
                  message.data,
                  message.err
                );
                if (call_map[message.corrID].fn["onStatus"]) {
                  ...
                  setTimeout(function () {
                    call_map[message.corrID] !== undefined &&
      ...
      call_map[corr_id].timer = setTimeout(function () {
        ...
        if (call_map[corr_id]) {
          for (var i in call_map[corr_id].fn) {
            typeof call_map[corr_id].fn[i] === "function" &&
              call_map[corr_id].fn[i]("timeout");
          }
  ...
  declareExchange(
    ...
    function (exc_got) {
      ...
      request_q = conn.queue(id, function (queueCreated) {
        log.debug(
          "Request queue for rpc server " + queueCreated.name + " is open"
        );
  ...
  declareExchange(
    ...
    function (exc_got) {
      ...
      msg_q = conn.queue("", function (queueCreated) {
        log.debug(
          "Message queue for topic participant is open:",
          queueCreated.name
        );
        ready = true;
        on_ready();
...
module.exports = function () {
  ...
  that.asTopicParticipant = function (group, on_ok, on_failure) {
    ...
    if (connection) {
      var topic_participant = topicParticipant(
        that,
        connection,
        group,
        function () {
