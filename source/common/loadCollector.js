    ...
    child_process.exec(
      ...
      function (err, stdout, stderr) {
        if (err) {
          ...
          var str_disk_info = lines[lines.length - 1].replace(
            /[\s\n\r]+/g,
            " "
          );
...
var networkCollector = function (period, interf, max_scale, on_load) {
  ...
  var reporter = setInterval(function () {
    var rt_load =
      Math.round(Math.max(rx_Mbps / max_scale, tx_Mbps / max_scale) * 1000) /
      1000;
    on_load(rt_load);
  }, period);
...
var gpuCollector = function (period, on_load) {
  ...
  var cpu_load = 0,
    cpu_collector = new cpuCollector(period, function (data) {
      cpu_load = data;
    });
  ...
  child.stdout.on("data", function (data) {
    ...
    for (; i < lines.length; i++) {
      ...
      for (var engine of engine_list) {
        ...
        if (
          ...
          if (engine_max_usage < engine_usage) engine_max_usage = engine_usage;
        }
      }
    ...
    if (samples > 0) load = usage_sum / samples / 100;
    else load = 0;
  });
