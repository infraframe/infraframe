...
module.exports = function (spec) {
  ...
  var rpcClient = spec.rpcClient,
    id =
      spec.purpose + '-' + genID() + '@' + (spec.info.hostname || spec.info.ip),
  ...
  var reportLoad = function (load) {
    ...
    previous_load = load;
    if (state === 'registered') {
      rpcClient.remoteCast(cluster_name, 'reportLoad', [id, load]);
    }

    if (
      load >
      0.98 /*FIXME: Introduce a configuration item to specify the overload threshold here.*/
    ) {
      on_overload();
    }
  };

  var load_collector = loadCollector({
    period: spec.loadCollection.period,
  ...
  var pickUpTasks = function (taskList) {
    rpcClient.remoteCast(cluster_name, 'pickUpTasks', [id, taskList]);
  };

  var layDownTask = function (task) {
    rpcClient.remoteCast(cluster_name, 'layDownTask', [id, task]);
  };

  var doRejectTask = function (task) {
    rpcClient.remoteCast(cluster_name, 'unschedule', [id, task]);
  };
  ...
  that.quit = function () {
    if (state === 'registered') {
      ...
      rpcClient.remoteCast(cluster_name, 'quit', [id]);
  ...
  that.reportState = function (st) {
    if (state === 'registered') {
      rpcClient.remoteCast(cluster_name, 'reportState', [id, st]);
    }
  };
