...
class RpcClient {
  ...
  setup() {
    ...
    return channel
      .assertExchange(RPC_EXC.name, RPC_EXC.type, RPC_EXC.options)
      .then(() => {
          ...
          (rawMessage) => {
            try {
              ...
              if (this.callMap[msg.corrID] !== undefined) {
                ...
                this.callMap[msg.corrID].fn[msg.type].call(
                  {},
                  msg.data,
                  msg.err
                );
  ...
  close() {
    ...
    this.callMap = {};
    if (this.consumerTag) {
      return this.bus.channel.cancel(this.consumerTag).catch((err) => {
          ...
          (rawMessage) => {
            try {
              ...
              if (typeof this.methods[msg.method] === "function") {
                ...
                log.warn(
                  "RPC server does not support this method:",
                  msg.method
                );
                if (msg.replyTo && msg.corrID !== undefined) {
                  const content = JSON.stringify({
...
class TopicParticipant {
  ...
  setup() {
    const channel = this.bus.channel;
    return channel
      .assertExchange(this.name, "topic", MONITOR_EXC.options)
      .then(() => {
  ...
  subscribe(patterns, onMessage, onOk) {
    ...
    if (this.queue && channel) {
      patterns.map((pattern) => {
        channel
          .bindQueue(this.queue, this.name, pattern)
        ...
        .consume(
          ...
          (rawMessage) => {
            try {
              ...
              log.error(
                "Error processing topic message:",
                rawMessage,
                "and error:",
                error
              );
  ...
  unsubscribe(patterns) {
    ...
    if (this.queue && channel) {
      patterns.map((pattern) => {
        channel
          .unbindQueue(this.queue, this.name, pattern)
  ...
  close() {
    this.ready = false;
    if (this.queue) {
      return this.bus.channel.deleteQueue(this.queue).catch((err) => {
...
class Monitor {
  ...
  setup() {
    const channel = this.bus.channel;
    return channel
      .assertExchange(MONITOR_EXC.name, MONITOR_EXC.type, MONITOR_EXC.options)
      .then(() => {
        return channel.assertQueue("", Q_OPTION);
      })
      .then((result) => {
        ...
        return channel.consume(
          ...
          (rawMessage) => {
            try {
              ...
              log.error(
                "Error processing monitored message:",
                rawMessage,
                "and error:",
                error
              );
...
class MonitoringTarget {
  ...
  notify(reason, message) {
    ...
    if (this.ready && channel) {
      ...
      try {
        channel.publish(MONITOR_EXC.name, pattern, Buffer.from(content));
      } catch (e) {
        log.warn("Failed to publish:", e);
      }
    } else {
...
class AmqpCli {
  ...
  asTopicParticipant(group, onOk, onFailure) {
    if (!this.topicParticipants.has(group)) {
      const topicParticipant = new TopicParticipant(this, group);
      topicParticipant
        .setup()
        .then(() => {
