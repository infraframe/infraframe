...
var ClusterManager = function (clusterName, selfId, spec) {
  ...
  var createScheduler = function (purpose) {
    var strategy = spec.strategy[purpose]
      ? spec.strategy[purpose]
      : spec.strategy.general;
    return new Scheduler({
      purpose: purpose,
      strategy: strategy,
      scheduleReserveTime: spec.scheduleReserveTime,
    });
  };
  ...
  var workerJoin = function (purpose, worker, info) {
    ...
    workers[worker] = { purpose: purpose, alive_count: 0 };
    ...
    data_synchronizer &&
      data_synchronizer({
        type: 'worker_join',
        payload: { purpose: purpose, worker: worker, info: info },
      });
    return state;
  };
  ...
  var workerQuit = function (worker) {
    ...
    if (workers[worker] && schedulers[workers[worker].purpose]) {
      ...
      data_synchronizer &&
        data_synchronizer({ type: 'worker_quit', payload: { worker: worker } });
    }
    ...
    if (purpose) {
      ...
      if (!clusterInfo[purpose]) {
        ...
        if (sendRequest) {
          log.info(
            'Send updateCapacity event remove to cloud with data:',
            data
          );
          send('POST', 'updateCapacity', data);
        }
  ...
  var reportState = function (worker, state) {
    workers[worker] &&
      schedulers[workers[worker].purpose] &&
      schedulers[workers[worker].purpose].updateState(worker, state);
    data_synchronizer &&
      data_synchronizer({
        type: 'worker_state',
        payload: { worker: worker, state: state },
      });
  };
  ...
  var reportLoad = function (worker, load) {
    ...
    workers[worker] &&
      schedulers[workers[worker].purpose] &&
      schedulers[workers[worker].purpose].updateLoad(worker, load);
    data_synchronizer &&
      data_synchronizer({
        type: 'worker_load',
        payload: { worker: worker, load: load },
      });
  };
  ...
  var pickUpTasks = function (worker, tasks) {
    workers[worker] &&
      schedulers[workers[worker].purpose] &&
      schedulers[workers[worker].purpose].pickUpTasks(worker, tasks);
    data_synchronizer &&
      data_synchronizer({
        type: 'worker_pickup',
        payload: { worker: worker, tasks: tasks },
      });
  };
  ...
  var layDownTask = function (worker, task) {
    workers[worker] &&
      schedulers[workers[worker].purpose] &&
      schedulers[workers[worker].purpose].layDownTask(worker, task);
    data_synchronizer &&
      data_synchronizer({
        type: 'worker_laydown',
        payload: { worker: worker, task: task },
      });
  };
      ...
      if (schedulers[purpose]) {
        schedulers[purpose].schedule(
          ...
          function (reason) {
            ...
            on_error(
              'Failed in scheduling ' + purpose + ' worker, reason: ' + reason
            );
          }
        );
      } else {
  ...
  var unschedule = function (worker, task) {
    workers[worker] &&
      schedulers[workers[worker].purpose] &&
      schedulers[workers[worker].purpose].unschedule(worker, task);
    data_synchronizer &&
      data_synchronizer({
        type: 'unscheduled',
        payload: { worker: worker, task: task },
  ...
  var getWorkerAttr = function (worker, on_ok, on_error) {
    if (workers[worker]) {
      var worker_info =
        schedulers[workers[worker].purpose] &&
        schedulers[workers[worker].purpose].getInfo(worker);
  ...
  var getTasks = function (worker, on_ok) {
    return workers[worker] && schedulers[workers[worker].purpose]
      ? schedulers[workers[worker].purpose].getTasks(worker)
      : [];
  };
  ...
  that.setUpdatedData = function (data) {
    ...
    switch (data.type) {
      ...
      case 'worker_laydown':
        ...
        break;
      case 'scheduled':
        schedulers[data.payload.purpose] &&
          schedulers[data.payload.purpose].setScheduled(
            data.payload.task,
            data.payload.worker,
            data.payload.reserve_time
          );
        break;
      case 'unscheduled':
...
class HeartbeatObserver {
  constructor(onLoss) {
    ...
    this.state = state;
    this.onLoss = onLoss;
    this.minStepCount = getTimerCount(
      parseInt(state.electTimeout),
      state.minHeartTicket
    );
    this.maxStepCount = getTimerCount(
      state.electTimeout * 2,
      state.minHeartTicket
    );
  ...
  resetTimer() {
    ...
    if (self.observerTimes > 1) {
      maxCount =
        Math.random() * (self.maxStepCount - self.minStepCount) +
        self.minStepCount;
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      self._waitingPromises.push(() => {
        ...
        if (delay > state.maxDelay) {
          log.debug(
            state.id,
            `${self.lockKey}.${self.lockIndex++} handler delay:`,
            `${delay}ms`,
            'leaderId:',
            state.leaderId,
            'self:',
...
var runAsFollower = function (topicChannel, manager) {
  ...
  var routerKeys = [
    'clusterManager.follower',
    `clusterManager.follower.${state.id}`,
  ];
  var observer = new HeartbeatObserver(async (loseTime, lastContact) => {
    ...
    try {
      ...
      log.fatal(
        ...
        lastContact,
        'timeout:',
        `${loseTime.getTime() - lastContact.getTime()}ms`
      );
  ...
  var installSnapshot = async function (data) {
    ...
    if (!isInstallSnapshot) {
      ...
      log.info(
        'start installSnapshot last commitId:',
        state.commitId,
        'data.commitId:',
        data.commitId,
        'from:',
      ...
      topicChannel.publish(`clusterManager.leader.${state.leaderId}`, {
  ...
  var responseVote = function (term, id, commitId, voteForTerm, voteGranted) {
    ...
    topicChannel.publish(`clusterManager.candidate.${id}`, {
      type: 'responseVote',
      data: {
        voteGranted,
        term,
  ...
  var needHandle = function (message) {
    if (
      message.type === 'heartbeart' ||
      message.type === 'installSnapshot' ||
      message.type === 'appendEntry'
    ) {
      ...
      if (delay >= state.maxDelay) {
        log.fatal(
          `[follower.${state.id}] got hear beat from [leader:${
            data.id
          }]: ${new Date(data.timestamp)}, delay: ${delay}ms`
        );
      } else {
        log.debug(
          `[follower.${state.id}] got hear beat from [leader:${
    ...
    try {
      ...
      if (message.type === 'installSnapshot') {
        ...
        if (data.followerId !== state.id) {
          log.fatal(
            'installSnapshot failed followerId:',
            data.followerId,
            'my id:',
            state.id
          );
        ...
        if (data.commitId < state.commitId) {
          log.fatal(
            'installSnapshot failed commitId:',
            data.commitId,
            'my commitId:',
            state.commitId
          );
        ...
        if (state.leaderId != 0 && state.leaderId != data.id) {
          log.warn(
            'follower got appendEntry no my leaderId',
            'data:',
            data,
            'self:',
            state
        ...
        if (state.term > data.term) {
          log.warn(
            'appendEntry ignore an older term:',
            data.term,
            'commitId:',
            data.commitId,
            'self:',
        ...
        if (state.leaderId != 0 && state.leaderId != data.id) {
          log.warn(
            'follower got heartbeart no my leaderId',
            'data:',
            data,
            'self:',
            state
        ...
        if (state.term > data.term) {
          log.warn(
            'follower heartbeart ignore an older term:',
            data.term,
            'commitId:',
            data.commitId,
            'self:',
        ...
        if (state.prevLogIndex != DEFAULT_DATA) {
          ...
          if (prevLogTerm > data.prevLogTerm) {
            log.warn(
              'follower heartbeart ignore revious log term mis-match prevLogTerm:',
              data.prevLogTerm,
              'commitId:',
              data.commitId,
              'self:',
        ...
        if (oldLeaderId != state.leaderId) {
          log.debug(
            'follower found new leader last commitId:',
            state.commitId,
            'data.commitId:',
            data.commitId,
            'from:',
        ...
        if (state.leaderId != 0 && state.leaderId != data.id) {
          log.warn(
            'follower rejecting vote request since we have a leader',
            'data:',
            data,
            'self:',
            state
        ...
        if (
          ...
          log.warn(
            'follower duplicate requestVote for same term:',
            data.term,
            'data:',
            data,
            'self:',
        ...
        if (state.prevLogIndex != DEFAULT_DATA) {
          ...
          if (prevLogTerm > data.prevLogTerm) {
            log.warn(
              'follower rejecting vote request since our prevLogTerm is greater',
              'data:',
              data,
              'self:',
              state
          ...
          if (
            ...
            log.warn(
              'follower rejecting vote request since our commitId is greater',
              'data:',
              data,
              'self:',
              state
        ...
        log.debug(
          'follower, not concerned message:',
          message,
          'routingKey:',
          routingKey,
          'self:',
  ...
  return new Promise((resolve) => {
    topicChannel.subscribe(routerKeys, onTopicMessage, function () {
      ...
      log.info(
        state.id,
        'Run as follower success leaderId:',
        state.leaderId,
        'self:',
        state
      );
...
var runAsLeader = function (topicChannel, manager) {
  ...
  var interval;
  var routerKeys = [
    'clusterManager.leader',
    `clusterManager.leader.${state.id}`,
  ];
  var observer = new HeartbeatObserver(async (loseTime, lastContact) => {
    ...
    try {
      ...
      log.fatal(
        'leader Lose heart-beat from self:',
        state.id,
        'loseTime:',
        loseTime,
        'lastContact:',
  ...
  var needHandle = function (message) {
    if (message.type === 'heartbeart') {
      ...
      if (data.id === state.id) {
        ...
        if (delay >= state.maxDelay) {
          log.fatal(
            `[leader.${state.id}] got hear beat from [leader:${
              data.id
            }]: ${new Date(data.timestamp)}, delay: ${delay}ms`
          );
        } else {
    ...
    try {
      ...
      if (message.type === 'installSnapshot') {
        ...
        manager.getRuntimeData(function (data) {
          ...
          topicChannel.publish(`clusterManager.follower.${followerId}`, {
            type: 'installSnapshot',
            data: data,
          });
        });
  ...
  var sendHeartbeat = function () {
    state.timestamp = new Date().getTime();
    topicChannel.publish('clusterManager.leader', {
      type: 'heartbeart',
      data: state,
    });
    topicChannel.publish('clusterManager.follower', {
      type: 'heartbeart',
      data: state,
    });
    topicChannel.publish('clusterManager.candidate', {
      type: 'heartbeart',
      data: state,
    });
  };
  ...
  return new Promise((resolve) => {
    topicChannel.subscribe(routerKeys, onTopicMessage, function () {
      broadcastOnTopicMsg = onTopicMessage;
      topicChannel.bus.asRpcServer(
        manager.name,
        manager.rpcAPI,
        (rpcSvr) => {
          log.info('Run as asRpcServer.');
          topicChannel.bus.asMonitoringTarget(
            function (monitoringTgt) {
            ...
            function (reason) {
              log.error(
                'Cluster leader running as monitoring target failed, reason:',
                reason
              );
        ...
        function (reason) {
          log.error(
            'Cluster leader running as RPC server failed, reason:',
            util.inspect(reason)
          );
          process.kill(process.pid, 'SIGINT');
        }
...
var runAsCandidate = function (topicChannel, manager) {
  ...
  var routerKeys = [
    ...
    `clusterManager.candidate.${state.id}`,
  ];
  ...
  var resetTimer = function (msg) {
    ...
    let timeout =
      Math.random() * (state.electTimeout * 2 - mixElecTimeout) +
      mixElecTimeout;
    timer = setTimeout(() => {
      electTimeout(msg);
    }, timeout);
  ...
  var onTopicMessage = async function (message) {
    ...
    try {
      ...
      if (message.type === 'requestVote') {
        ...
        if (data.term < state.term) {
          log.warn(
            'candidate ignore an older term',
            'data:',
            data,
            'self:',
            state
        ...
        if (data.term > state.term) {
          log.warn(
            'candidate receive a newer term i run as follower',
            'data:',
            data,
            'self:',
            state
        ...
        if (
          ...
          log.warn(
            'candidate duplicate requestVote for same term:',
            data.term,
            'data:',
            data,
            'self:',
        ...
        if (state.prevLogIndex != DEFAULT_DATA) {
          ...
          if (prevLogTerm > data.prevLogTerm) {
            log.warn(
              'candidate rejecting vote request since our prevLogTerm is greater',
              'data:',
              data,
              'self:',
              state
          ...
          if (
            ...
            log.warn(
              'candidate rejecting vote request since our commitId is greater',
              'data:',
              data,
              'self:',
              state
        ...
        if (
          ...
          data.id == state.id &&
          data.voteForTerm == state.term
        ) {
          resetTimer('responseVote change as follower');
          log.warn(
            'candidate newer term discovered, fallback to follower',
            'data:',
            data,
            'self:',
            state
        ...
        if (state.prevLogIndex != DEFAULT_DATA) {
          ...
          if (prevLogTerm > data.prevLogTerm) {
            log.warn(
              'candidate rejecting run as follower since our prevLogTerm is greater',
              'data:',
              data,
              'self:',
              state
          ...
          if (
            ...
            log.warn(
              'candidate rejecting run as follower commitId is greater',
              'data:',
              data,
              'self:',
              state
