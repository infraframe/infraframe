...
var ClusterManager = function (clusterName, selfId, spec) {
  var that = { name: clusterName, id: selfId };
...
var runAsSlave = function (topicChannel, manager) {
  ...
  var requestRuntimeData = function () {
    topicChannel.publish("clusterManager.master", {
      type: "requestRuntimeData",
      data: manager.id,
    });
  };
  ...
  topicChannel.subscribe(
    ["clusterManager.slave.#", "clusterManager.*." + manager.id],
    onTopicMessage,
    function () {
      requestRuntimeData();
      superviseMaster();
...
var runAsMaster = function (topicChannel, manager) {
  ...
  topicChannel.bus.asRpcServer(
      ...
      topicChannel.bus.asMonitoringTarget(
        function (monitoringTgt) {
          ...
          setInterval(function () {
            ...
            topicChannel.publish("clusterManager.slave", {
              type: "declareMaster",
              data: { id: manager.id, life_time: life_time },
            });
          ...
          var onTopicMessage = function (message) {
            ...
            if (message.type === "requestRuntimeData") {
              ...
              manager.getRuntimeData(function (data) {
                topicChannel.publish("clusterManager.slave." + from, {
                  type: "runtimeData",
                  data: data,
                });
              });
            } else if (
              message.type === "declareMaster" &&
              message.data.id !== manager.id
            ) {
              log.error(
                "!!Double master!! self:",
                manager.id,
        ...
        function (reason) {
          log.error(
            "Cluster manager running as monitoring target failed, reason:",
            reason
          );
          process.kill(process.pid, "SIGINT");
        }
    ...
    function (reason) {
      log.error(
        "Cluster manager running as RPC server failed, reason:",
        reason
      );
      process.kill(process.pid, "SIGINT");
    }
