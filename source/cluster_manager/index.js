...
function startup() {
  ...
  var enableService = function () {
    amqper.asTopicParticipant(
      config.manager.name + ".management",
      function (channel) {
        log.info("Cluster manager up! id:", id);
        clusterManager = new ClusterManager.manager(
          channel,
          config.manager.name,
          id,
          spec
  ...
  amqper.connect(
    ...
    function (reason) {
      log.error(
        "Cluster manager connect to rabbitMQ server failed, reason:",
        reason
      );
      process.kill(process.pid, "SIGINT");
    }
