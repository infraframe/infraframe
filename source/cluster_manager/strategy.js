...
var lastUsed = function () {
  ...
  this.allocate = function (workers, candidates, on_ok, on_error) {
    if (
      last_used === undefined ||
      workers[last_used] === undefined ||
      candidates.indexOf(last_used) === -1
    ) {
...
var roundRobin = function () {
  ...
  this.allocate = function (workers, candidates, on_ok, on_error) {
    ...
    if (i === -1) {
      ...
      latest_used =
        i === candidates.length - 1 ? candidates[0] : candidates[i + 1];
    }
    on_ok(latest_used);
  };
};
...
exports.create = function (strategy) {
  switch (strategy) {
    ...
    default:
      log.warn(
        "Invalid specified scheduling strategy:",
        strategy,
        ', apply "randomly-pick" instead.'
      );
      return new randomlyPick();
  }
};
