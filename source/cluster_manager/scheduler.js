...
var isWorkerAvailable = function (worker) {
  return (
    worker.load < worker.info.maxLoad &&
    (worker.state === undefined || worker.state === 2)
  );
};
...
exports.Scheduler = function (spec) {
    ...
    if (workers[worker]) {
      ...
      if (tasks[task]) {
        ...
        if (tasks[task].worker !== worker) {
          log.warn(
            'Worker conflicts for task:',
            task,
            'and update to worker:',
            worker
          );
          tasks[task].worker = worker;
        }
      } else {
        tasks[task] = { reserve_time: schedule_reserve_time, worker: worker };
      }
    }
  };
  ...
  var isTaskInExecution = function (task, worker) {
    return (
      ...
      workers[worker].tasks.indexOf(task) !== -1
    );
  };
  ...
  that.schedule = function (task, preference, reserveTime, on_ok, on_error) {
    if (tasks[task]) {
      var newReserveTime =
          reserveTime && tasks[task].reserve_time < reserveTime
            ? reserveTime
            : tasks[task].reserve_time,
        worker = tasks[task].worker;
      ...
      strategy.allocate(
        ...
        function (worker) {
          reserveWorkerForTask(
            task,
            worker,
            reserveTime && reserveTime > 0 ? reserveTime : schedule_reserve_time
          );
  ...
  that.setScheduled = function (task, worker, reserveTime) {
    tasks[task] = { worker: worker, reserve_time: reserveTime };
  };
  ...
  that.getData = function () {
    ...
    for (var task in tasks) {
      data.tasks[task] = {
        reserve_time: tasks[task].reserve_time,
        worker: tasks[task].worker,
      };
    }
    return data;
