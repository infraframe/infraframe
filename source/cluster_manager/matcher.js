...
var is_isp_applicable = function (supported_isps, preferred_isp) {
  return (
    supported_isps.indexOf(preferred_isp) !== -1 || supported_isps.length === 0
  );
};
...
var videoMatcher = function () {
  this.match = function (preference, workers, candidates) {
    if (!preference || !preference.video) return candidates;

    var formatContain = function (listA, listB) {
      var count = 0;
      listB.forEach((fmtB) => {
        if (listA.indexOf(fmtB) > -1) count++;
      });
    ...
    var availableCandidates = candidates.filter(function (cid) {
      ...
      var decodeOk = false;
      if (capacity.video) {
        encodeOk = formatContain(
          capacity.video.encode,
          preference.video.encode
        );
        decodeOk = formatContain(
          capacity.video.decode,
          preference.video.decode
        );
      }
      if (!encodeOk) {
        log.warn(
          "No available workers for encoding:",
          JSON.stringify(preference.video.encode)
        );
      }
      if (!decodeOk) {
        log.warn(
          "No available workers for decoding:",
          JSON.stringify(preference.video.decode)
        );
      }
...
var analyticsMatcher = function () {
  this.match = function (preference, workers, candidates) {
    if (!preference || !preference.algorithm) return candidates;
...
exports.create = function (purpose) {
  switch (purpose) {
    ...
    default:
      log.warn(
        "Invalid specified purpose:",
        purpose,
        ", apply general-matcher instead."
      );
      return new generalMatcher();
  }
};
