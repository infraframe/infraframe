...
const clientInfo = {
  sdk: { version: "3.3", type: "JavaScript" },
  runtime: { name: "Chrome", version: "53.0.0.0" },
  os: { name: "Linux (Ubuntu)", version: "14.04" },
};
// JavaScript SDK <3.3 does not support reconnection. So use iOS UA info for reconnection tests.
const reconnectionClientInfo = {
  sdk: { version: "3.3", type: "Objective-C" },
  runtime: { name: "WebRTC", version: "54" },
  os: { name: "iPhone OS", version: "10.2" },
};
const jsLoginInfo = {
  token: new Buffer(JSON.stringify("someValidToken")).toString("base64"),
  ...
  it("Connecting to an insecure socket.io server should succeed.", function (done) {
    ...
    server
      ...
      .then(function (result) {
        ...
        var client = sioClient.connect("http://localhost:3001", {
          reconnect: false,
          secure: false,
  ...
  it("Connecting to a secured socket.io server should succeed.", function (done) {
    ...
    server
      ...
      .then(function (result) {
        ...
        var client = sioClient.connect("https://localhost:3005", {
          reconnect: false,
          secure: true,
          "force new connection": true,
...
describe("Logining and Relogining.", function () {
  ...
  describe("on: login", function () {
    beforeEach("Clients connect to the server.", function (done) {
      client = sioClient.connect(serverUrl, {
        reconnect: false,
        secure: false,
        "force new connection": true,
      });
    ...
    let someValidToken = new Buffer(JSON.stringify("someValidToken")).toString(
      "base64"
    );
    ...
    it("Joining with valid token and UA info should succeed.", function (done) {
      ...
      var transformed_participants = [
        { id: "participant-id", name: "user-id-2", role: "presenter" },
      ];

      client.emit(
        "login",
        { token: someValidToken, userAgent: reconnectionClientInfo },
        function (status, resp) {
          expect(status).to.equal("success");
          expect(mockPortal.join.getCall(0).args).to.deep.equal([
            client.id,
            "someValidToken",
          ]);
          expect(mockServiceObserver.onJoin.getCall(0).args).to.deep.equal([
            "tokenCode",
          ]);
          ...
          var reconnection_ticket = JSON.parse(
            new Buffer(resp.reconnectionTicket, "base64").toString()
          );
    ...
    it("Joining with invalid token and valid UA info should fail and cause disconnection.", function (done) {
      ...
      client.emit(
        "login",
        {
          token: new Buffer(JSON.stringify("someInvalidToken")).toString(
            "base64"
          ),
          userAgent: clientInfo,
        },
    ...
    it("Joining with token which is not base64 encoded should fail and cause disconnection.", function (done) {
      ...
      client.emit(
        "login",
        { token: "someInvalidToken", userAgent: clientInfo },
        function (status, resp) {
          expect(status).to.equal("error");
    ...
    it("Joining with valid tokens and invalid UA should fail and cause disconnection.", function (done) {
      ...
      client.emit(
        "login",
        { token: someValidToken, userAgent: "invalidUserAgent" },
        function (status, resp) {
          expect(status).to.equal("error");
    ...
    xit("Do not issue reconnection ticket to JavaScript clients.", function (done) {
      ...
      client.emit(
        "login",
        { token: someValidToken, userAgent: clientInfo },
        function (status, resp) {
          expect(status).to.equal("success");
  ...
  describe("on: relogin", function () {
    ...
    beforeEach("Clients connect to the server.", function (done) {
      reconnection_client = sioClient.connect(serverUrl, {
        reconnect: true,
        secure: false,
        "force new connection": true,
      });
    ...
    const someValidLoginInfo = {
      token: new Buffer(JSON.stringify("someValidToken")).toString("base64"),
      userAgent: reconnectionClientInfo,
    };
    ...
    it("Relogin with correct ticket should success.", function (done) {
      ...
      const reconnection = function () {
        return new Promise(function (resolve) {
          reconnection_client.connect(serverUrl, {
            reconnect: false,
            secure: false,
    ...
    xit("Relogin with correct ticket after logout should fail.", function (done) {
      ...
      reconnection_client.emit(
        ...
        function (status, resp) {
          expect(status).to.equal("success");
          setTimeout(function () {
            // Postpone disconnect. Otherwise, disconnect will be executed before establishing connection.
    ...
    it("Dropped user cannot be reconnected.", function (done) {
      let client = sioClient.connect(serverUrl, {
        reconnection: true,
        secure: false,
        "force new connection": false,
      });
    ...
    it("Reconnect before original socket is disconnected is allowed.", function (done) {
      let client1 = sioClient.connect(serverUrl, {
        reconnection: true,
        secure: false,
        "force new connection": false,
      });
      client1.on("connect", function () {
        ...
        client1.emit("login", someValidLoginInfo, function (status, resp) {
          expect(status).to.equal("success");
          let client2 = sioClient.connect(serverUrl, {
            reconnection: true,
            secure: false,
            "force new connection": false,
          });
      ...
      client1.on("connect", function () {
        ...
        client1.emit("login", someValidLoginInfo, function (status, resp) {
          ...
          let options = {
            state: "erizo",
            audio: true,
            video: { resolution: "vga", framerate: 30, device: "camera" },
          };
          client1.emit("publish", options, undefined, function (status, id) {
            ...
            client2.emit(
              ...
              function (status, resp) {
                expect(status).to.equal("success");
                server.notify(participant_id, "progress", {
                  id: stream_id,
                  status: "soac",
                  data: { type: "candidate", candidate: "I'm a candidate." },
                });
              }
            );
          });
  ...
  describe("on: disconnect", function () {
    ...
    xit("Disconnecting after joining should cause participant leaving.", function (done) {
      ...
      client.on("disconnect", function () {
        ...
        setTimeout(function () {
          expect(mockPortal.leave.getCall(0).args).to.deep.equal([
            participant_id,
          ]);
          expect(mockServiceObserver.onLeave.getCall(0).args).to.deep.equal([
            "tokenCode",
          ]);
          done();
        }, 100);
...
describe("Notifying events to clients.", function () {
  ...
  it("Notifying the unjoined clients should fail.", function () {
    return expect(
      server.notify("112233445566", "event", "data")
    ).to.be.rejectedWith("participant does not exist");
  });

  it("Notifying the joined clients should succeed.", function (done) {
    var client = sioClient.connect(serverUrl, {
      reconnect: false,
      secure: false,
      "force new connection": true,
    });
    ...
    client.on("user_join", function (data) {
      expect(data).to.deep.equal({
        user: { id: "participantId", name: "user-name", role: "presenter" },
      });
    ...
    client.on("connect", function () {
      ...
      client.emit("login", jsLoginInfo, function (status, resp) {
        ...
        expect(status).to.equal("success");
        return server
          .notify(client.id, "participant", {
            action: "join",
            data: { id: "participantId", user: "user-name", role: "presenter" },
          })
          .then((result) => {
            expect(result).to.equal("ok");
            return server.notify(client.id, "stream", {
              status: "add",
              id: "streamId",
              data: {
                id: "streamId",
          ...
          .then((result) => {
            ...
            return server.notify(client.id, "stream", {
              status: "update",
              id: "streamId",
              data: { field: "video.status", value: "inactive" },
            });
          })
          .then((result) => {
            expect(result).to.equal("ok");
          ...
          .then((result) => {
            expect(result).to.equal("ok");
            return server.notify(client.id, "participant", {
              action: "leave",
              data: "participantId",
            });
          });
...
describe("Responding to clients.", function () {
  ...
  describe("on: publish, signaling_message", function () {
    it("Publishing webrtc streams after joining should succeed.", function (done) {
      ...
      joinFirstly().then(function (result) {
        expect(result).to.equal("ok");
        var options = {
          state: "erizo",
          audio: true,
        ...
        client.emit("publish", options, undefined, function (status, id) {
          ...
          client.emit(
            ...
            {
              streamId: stream_id,
              msg: { type: "offer", sdp: "offerSDPString" },
            },
            undefined,
            function () {
              expect(
                mockPortal.onSessionSignaling.getCall(0).args
              ).to.deep.equal([
                client.id,
                stream_id,
                { type: "offer", sdp: "offerSDPString" },
              ]);
              client.emit(
                "signaling_message",
                {
                  streamId: stream_id,
                  msg: { type: "candidate", candidate: "candidateString" },
                },
                undefined,
                function () {
                  expect(
                    mockPortal.onSessionSignaling.getCall(1).args
                  ).to.deep.equal([
                    client.id,
                    stream_id,
                    { type: "candidate", candidate: "candidateString" },
                  ]);
                  server.notify(client.id, "progress", {
                    id: stream_id,
                    status: "ready",
                  });
                }
        ...
        client.on("signaling_message_erizo", function (arg) {
          if (arg.mess.type === "ready") {
            ...
            expect(mockPortal.streamControl.getCall(0).args).to.deep.equal([
              client.id,
              stream_id,
              { operation: "mix", data: "common" },
            ]);
          }
        });
      });
          ...
          client.emit("publish", options2, undefined, function (status, id) {
            ...
            client.emit("publish", options3, undefined, function (status, id) {
              ...
              var options4 = {
                state: "erizo",
                audio: false,
                video: { resolution: "vga", device: "screen" },
              };
              client.emit(
                "publish",
                options4,
                ...
                function (status, id) {
                  ...
                  client.emit(
                    ...
                    function (status, id) {
                      expect(
                        mockPortal.publish.getCall(4).args[2]
                      ).to.deep.equal({
                        type: "streaming",
                        connection: {
                          url: "urlOfRtspOrRtmpSource",
  ...
  describe("on: unpublish", function () {
    it("Unpublishing streams should succeed if portal.unpublish succeeds.", function (done) {
      ...
      joinFirstly().then(function (result) {
        ...
        client.emit(
          "unpublish",
          "previouslyPublishedStreamId",
          function (status, data) {
            expect(status).to.equal("success");
            expect(mockPortal.unpublish.getCall(0).args).to.deep.equal([
              client.id,
              "previouslyPublishedStreamId",
            ]);
            done();
  ...
  describe("on: mix, unmix", function () {
    it("Mixing/unmixing streams should succeed if portal.streamControl suceeds.", function (done) {
      ...
      joinFirstly().then(function () {
        client.emit(
          "mix",
          { streamId: "streamId", mixStreams: ["mix-view1", "mix-view2"] },
          function (status, data) {
            expect(status).to.equal("success");
            expect(mockPortal.streamControl.getCall(0).args).to.deep.equal([
              client.id,
              "streamId",
              { operation: "mix", data: "view1" },
            ]);
            expect(mockPortal.streamControl.getCall(1).args).to.deep.equal([
              client.id,
              "streamId",
              { operation: "mix", data: "view2" },
            ]);
            ...
            client.emit(
              ...
              function (status, data) {
                expect(status).to.equal("success");
                expect(mockPortal.streamControl.getCall(2).args).to.deep.equal([
                  client.id,
                  "streamId",
                  { operation: "unmix", data: "view1" },
                ]);
                expect(mockPortal.streamControl.getCall(3).args).to.deep.equal([
                  client.id,
                  "streamId",
                  { operation: "unmix", data: "view2" },
                ]);
                done();
              }
            );
    ...
    it("Subscribing with proper options after joining should succeed.", function (done) {
      ...
      joinFirstly().then(function (result) {
        ...
        client.emit("subscribe", options, undefined, function (status, id) {
          ...
          expect(mockPortal.subscribe.getCall(0).args[2]).to.deep.equal({
            type: "webrtc",
            media: {
              audio: { from: "targetStreamId" },
              video: {
                from: "targetStreamId",
                parameters: {
                  resolution: { width: 640, height: 480 },
      ...
      joinFirstly().then(function (result) {
        ...
        client.emit("subscribe", options, undefined, function (status, id) {
          ...
          client.emit("subscribe", options1, undefined, function (status, id) {
            ...
            var options2 = {
              streamId: "targetStreamId2",
              video: { resolution: "vga", quality_level: "standard" },
            };
            client.emit(
              "subscribe",
              options2,
        ...
        client.emit("addExternalOutput", options, function (status, data) {
          ...
          expect(mockPortal.subscribe.getCall(0).args[2]).to.deep.equal({
            ...
            media: {
              audio: {
                from: "targetStreamId",
                format: { codec: "aac", sampleRate: 48000, channelNum: 2 },
              },
              video: {
                from: "targetStreamId",
                format: { codec: "h264" },
                parameters: { resolution: { width: 640, height: 480 } },
              },
            },
            connection: { url: "rtsp://target.host" },
          });
          ...
          client.emit("addExternalOutput", options, function (status, data) {
            ...
            expect(mockPortal.subscribe.getCall(1).args[2]).to.deep.equal({
              type: "streaming",
              media: {
                audio: false,
                video: {
              ...
              connection: { url: "rtsp://target2.host" },
            });
            ...
            options = {
              ...
              audio: true,
              resolution: { width: 640, height: 480 },
              url: "rtsp://target3.host",
            };
            client.emit("addExternalOutput", options, function (status, data) {
              expect(status).to.equal("success");
              expect(data.url).to.equal("rtsp://target3.host");
              expect(mockPortal.subscribe.getCall(2).args[0]).to.equal(
                client.id
              );
              expect(mockPortal.subscribe.getCall(2).args[1]).to.be.a("string");
              expect(mockPortal.subscribe.getCall(2).args[2]).to.deep.equal({
                type: "streaming",
                media: {
                  audio: {
                    from: "targetStreamId",
                ...
                connection: { url: "rtsp://target3.host" },
              });
              done();
            });
          });
          ...
          expect(mockPortal.subscribe.getCall(0).args[2]).to.deep.equal({
            ...
            media: {
              ...
              video: {
                from: testStream,
                format: { codec: "h264" },
                parameters: { resolution: { width: 1280, height: 720 } },
              },
            },
            connection: { url: "rtsp://target.host" },
          });
        ...
        client.emit("addExternalOutput", options, function (status, data) {
          ...
          client.emit(
            ...
            function (status, data) {
              ...
              expect(data.url).to.equal("rtsp://target.host");
              expect(
                mockPortal.subscriptionControl.getCall(0).args[0]
              ).to.equal(client.id);
              expect(mockPortal.subscriptionControl.getCall(0).args[1]).to.be.a(
                "string"
              );
              expect(
                mockPortal.subscriptionControl.getCall(0).args[2]
              ).to.deep.equal({
                operation: "update",
                data: {
                  audio: { from: "stream1" },
  ...
  describe("on: startRecorder", function () {
    it("Starting recording after joining with proper options should succeed.", function (done) {
      ...
      joinFirstly().then(function (result) {
        ...
        client.emit("startRecorder", options, function (status, data) {
          ...
          expect(mockPortal.subscribe.getCall(0).args[0]).to.equal(client.id);
          expect(mockPortal.subscribe.getCall(0).args[1]).to.equal(
            data.recorderId
          );
          expect(mockPortal.subscribe.getCall(0).args[2]).to.deep.equal({
            type: "recording",
            media: {
              audio: { from: "targetStreamId-01", format: { codec: "pcmu" } },
              video: { from: "targetStreamId-02", format: { codec: "vp8" } },
            },
            connection: { container: "auto" },
          });

          options = {
            audioStreamId: testStream,
            videoStreamId: testStream,
            audioCodec: "pcmu",
            videoCodec: "vp8",
            path: "/tmp",
            interval: 1000,
          ...
          client.emit("startRecorder", options, function (status, data) {
            ...
            expect(data.host).to.equal("unknown");
            expect(mockPortal.subscribe.getCall(1).args[0]).to.equal(client.id);
            expect(mockPortal.subscribe.getCall(1).args[1]).to.equal(
              data.recorderId
            );
            expect(mockPortal.subscribe.getCall(1).args[2]).to.deep.equal({
              type: "recording",
              media: {
                audio: { from: testStream, format: { codec: "pcmu" } },
                video: { from: testStream, format: { codec: "vp8" } },
              },
              connection: { container: "auto" },
            });
            done();
    ...
    it("Exceptions should cause aborting recording and notifying clients.", function (done) {
      ...
      client.on("remove_recorder", function (data) {
        ...
        expect(mockPortal.unsubscribe.getCall(0).args).to.deep.equal([
          client.id,
          recorder_id,
        ]);
        done();
      ...
      joinFirstly().then(function (result) {
        expect(result).to.equal("ok");
        var options = {
          audioStreamId: "targetStreamId1",
          videoStreamId: "targetStreamId2",
          audioCodec: "pcmu",
          videoCodec: "vp8",
          path: "/tmp",
          interval: 1000,
      ...
      joinFirstly().then(function (result) {
        ...
        client.emit("startRecorder", options, function (status, data) {
          ...
          var options = {
            audioStreamId: "targetStreamId1",
            audioCodec: "opus",
          }; //unspecified video stream-id, audio codec is 'opus'.
          client.emit("startRecorder", options, function (status, data) {
            expect(status).to.equal("success");
            expect(mockPortal.subscribe.getCall(1).args[2]).to.deep.equal({
              type: "recording",
            ...
            client.emit("startRecorder", options, function (status, data) {
              ...
              expect(mockPortal.subscribe.getCall(2).args[2]).to.deep.equal({
                ...
                media: {
                  audio: false,
                  video: { from: "targetStreamId2", format: { codec: "h264" } },
                },
                connection: { container: "auto" },
              });
              done();
        ...
        joinFirstly().then(function (result) {
          startInitialRecording(function (initialRecorderId) {
            ...
            client.emit("startRecorder", options, function (status, data) {
              ...
              expect(
                mockPortal.subscriptionControl.getCall(0).args
              ).to.deep.equal([
                client.id,
                initialRecorderId,
                { operation: "update", data: { audio: { from: "stream1" } } },
              ]);

              var options = {
                recorderId: initialRecorderId,
                videoStreamId: "stream2",
              };
              client.emit("startRecorder", options, function (status, data) {
                expect(status).to.equal("success");
                expect(data.recorderId).to.equal(initialRecorderId);
                expect(
                  mockPortal.subscriptionControl.getCall(1).args
                ).to.deep.equal([
                  client.id,
                  initialRecorderId,
                  { operation: "update", data: { video: { from: "stream2" } } },
                ]);
                done();
              });
            });
          ...
          startInitialRecording(function (initialRecorderId) {
            ...
            client.emit("startRecorder", options, function (status, data) {
              ...
              expect(data).to.equal(
                "Neither audio.from nor video.from was specified"
              );
  ...
  describe("on: mute, unmute", function () {
    ...
    it("mute/unmute should succeed if portal.streamControl succeeds.", function (done) {
      ...
      joinFirstly().then(function (result) {
        ...
        client.emit(
          "mute",
          { streamId: "subStreamId", track: "video" },
          function (status, data) {
            expect(status).to.equal("success");
            expect(mockPortal.streamControl.getCall(0).args).to.deep.equal([
              client.id,
              "subStreamId",
              { operation: "pause", data: "video" },
            ]);
        ...
        client.emit(
          ...
          function (status, data) {
            ...
            expect(mockPortal.streamControl.getCall(0).args).to.deep.equal([
              client.id,
              "subStreamId",
              { operation: "get-region", data: "common" },
            ]);
            ...
            client.emit(
              ...
              function (status, data) {
                ...
                client.emit(
                  "getRegion",
                  { id: "subStreamId", mixStreamId: "mix-view1" },
                  function (status, data) {
                    expect(status).to.equal("success");
                    expect(data.region).to.equal("regionId");
                    expect(
                      mockPortal.streamControl.getCall(0).args
                    ).to.deep.equal([
                      client.id,
                      "subStreamId",
                      { operation: "get-region", data: "view1" },
                    ]);
  ...
  describe("on: customMessage", function () {
    it("Should fail if message type is invalid.", function (done) {
      ...
      joinFirstly().then(function (result) {
        ...
        client.emit(
          "customMessage",
          { type: 9898, receiver: "all", data: "Hi, there!" },
          function (status, data) {
            expect(status).to.equal("error");
            ...
            client.emit(
              ...
              {
                type: { obj: true },
                payload: { streamId: "streamId", action: "video-out-off" },
              },
              function (status, data) {
                expect(status).to.equal("error");
                expect(data).to.equal("Invalid message type");
    ...
    it("Should fail if message type is #data# and receiver is invalid.", function (done) {
      ...
      joinFirstly().then(function (result) {
        ...
        client.emit(
          "customMessage",
          { type: "data", receiver: "", data: "Hi, there!" },
          function (status, data) {
            expect(status).to.equal("error");
            ...
            client.emit(
              ...
              function (status, data) {
                ...
                client.emit(
                  "customMessage",
                  { type: "data", receiver: { obj: true } },
                  function (status, data) {
                    expect(status).to.equal("error");
                    expect(data).to.equal("Invalid receiver");
                    expect(mockPortal.text.callCount).to.equal(0);
                    expect(mockPortal.streamControl.callCount).to.equal(0);
                    expect(mockPortal.subscriptionControl.callCount).to.equal(
                      0
                    );
                    done();
                  }
          ...
          function (status, data) {
            ...
            client.emit(
              ...
              function (status, data) {
                ...
                client.emit(
                  "customMessage",
                  { type: "control", payload: { streamId: 246 } },
                  function (status, data) {
                    expect(status).to.equal("error");
                                      ...
                                      function (status, data) {
                                        ...
                                        client.emit(
                                          ...
                                          function (status, data) {
                                            ...
                                            expect(
                                              mockPortal.text.callCount
                                            ).to.equal(0);
                                            expect(
                                              mockPortal.streamControl.callCount
                                            ).to.equal(0);
                                            expect(
                                              mockPortal.subscriptionControl
                                                .callCount
                                            ).to.equal(0);
                                            done();
                                          }
    ...
    it("Should succeed if message type is #data# and portal.text succeeds.", function (done) {
      ...
      joinFirstly().then(function (result) {
        ...
        client.emit(
          "customMessage",
          { type: "data", receiver: "all", data: "Hi, there!" },
          function (status, data) {
            expect(status).to.equal("success");
            expect(mockPortal.text.getCall(0).args).to.deep.equal([
              client.id,
              "all",
              "Hi, there!",
            ]);
      ...
      joinFirstly().then(function (result) {
        ...
        client.emit("subscribe", options, undefined, function (status, id) {
          ...
          client.emit(
            ...
            {
              type: "control",
              payload: { streamId: "streamId", action: "video-in-off" },
            },
            function (status, data) {
              expect(status).to.equal("success");
              expect(
              ...
              client.emit("publish", options, undefined, function (status, id) {
                ...
                client.emit(
                  ...
                  {
                    type: "control",
                    payload: { streamId: pub_id, action: "video-out-off" },
                  },
                  function (status, data) {
                    expect(status).to.equal("success");
                    expect(
