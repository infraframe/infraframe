...
describe("rpcRequest.getController", function () {
  it("Should succeed if scheduling agent and requiring node succeed.", function () {
    ...
    mockRpcChannel.makeRPC
      .onCall(1)
      .returns(Promise.resolve("RpcIdOfController"));
  ...
  it("Should fail if scheduling agent error or timeout occurs.", function () {
    ...
    mockRpcChannel.makeRPC
      .onCall(0)
      .returns(Promise.reject("error or timeout while getting agent"));

    var req = rpcRequest(mockRpcChannel);

    return expect(
      req.getController("owt-cluster", "roomId")
    ).to.be.rejectedWith("error or timeout while getting agent");
  });
  ...
  it("Should fail if requiring node error or timeout occurs.", function () {
    ...
    mockRpcChannel.makeRPC
      .onCall(0)
      .returns(Promise.resolve("RpcIdOfControllerAgent"));
    mockRpcChannel.makeRPC
      .onCall(1)
      .returns(Promise.reject("error or timeout"));

    var req = rpcRequest(mockRpcChannel);

    return expect(
      req.getController("owt-cluster", "roomId")
    ).to.be.rejectedWith("error or timeout");
  });
});
...
describe("rpcRequest.join/leave/publish/unpublish/subscribe/unsubscribe/onSessionSignaling/streamControl/subscriptionControl/text", function () {
  it("Should succeed if rpcChannel.makeRPC succeeds.", function () {
    ...
    var join = req.join("rpcIdOfController", "roomId", {
      id: "participantId",
      user: "UserId",
      role: "UserRole",
      portal: "portalRpcId",
    ...
    var sub = req.subscribe(
      "rpcIdOfController",
      "participantId",
      "subscriptionId",
      "subOptions"
    );
    var unsub = req.unsubscribe(
      "rpcIdOfController",
      "participantId",
      "subscriptionId"
    );
    ...
    var streamControl = req.streamControl("rpcIdOfAccessNode", "streamId", {
      operation: "pause",
      data: "video",
    });
    var subscriptionControl = req.subscriptionControl(
      "rpcIdOfAccessNode",
      "subscriptionId",
      { operation: "update", data: { video: { from: "streamxx" } } }
    );
    var text = req.text(
      "rpcIdOfController",
      "fromWhom",
