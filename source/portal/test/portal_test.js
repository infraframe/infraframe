...
const testTokenKey =
  "6d96729ffd07202e7a65ac81278c43a2c87fc6367736431e614607e9d3ceee201f9680cdfcb88df508829e9810c46aaf02c9cc8dcf46369de122ee5b22ec963c";
const testSelfRpcId = "portal-38A87euk9kfh";
const testClusterName = "owt-cluster";
const testPortalSpec = {
  tokenKey: testTokenKey,
  clusterName: testClusterName,
  selfRpcId: testSelfRpcId,
};
const testToken = {
  tokenId: "573eab88111478bb3526421b",
  host: "10.239.158.161:8080",
  secure: true,
  signature:
    "ZTIwZTI0YzYyMDFmMTQ1OTc3ZTBkZTdhMDMyZTJhYTkwMTJiZjNhY2E5OTZjNzA1Y2ZiZGU1ODhjMDY3MmFkYw==",
};
const testParticipantId = "/#gpMeAEuPeMMDT0daAAAA";
const testRoom = "573eab78111478bb3526421a";

const delete_token_result = {
  code: "tokenCode",
  user: "user-id",
  role: "presenter",
...
describe("portal.updateTokenKey: update the token key.", function () {
  it("Joining with a valid token will fail if the token key is out of time, and success when token key is updated", function () {
    ...
    var portalSpecWitpauseTokenKey = {
      tokenKey: "OldTokenKey",
      clusterName: testClusterName,
      selfRpcId: testSelfRpcId,
    };
    ...
    var final_result = {
      tokenCode: delete_token_result.code,
      data: {
        user: delete_token_result.user,
        role: delete_token_result.role,
        permission: join_room_result.permission,
        room: join_room_result.room,
      },
    };

    return portal.join(testParticipantId, testToken).then(
      function (runInHere) {
      ...
      function (reason) {
        ...
        return portal
          .join(testParticipantId, testToken)
...
describe("portal.join: Participants join.", function () {
  it("Joining with valid token should succeed.", function () {
    ...
    return portal.join(testParticipantId, testToken).then(function (result) {
      expect(result).to.deep.equal(final_result);
      expect(mockrpcReq.getController.getCall(0).args).to.deep.equal([
        "owt-cluster",
        testRoom,
      ]);
      expect(mockrpcReq.join.getCall(0).args).to.deep.equal([
        "rpcIdOfController",
        testRoom,
        {
          ...
          user: delete_token_result.user,
          role: delete_token_result.role,
          portal: testSelfRpcId,
          origin: delete_token_result.origin,
        },
      ]);
    });
  });
  ...
  it("The same participant re-joining should succeed.", function () {
    ...
    return portal.join(testParticipantId, testToken).then(function (result) {
      expect(result).to.deep.equal(final_result);
      expect(portal.join(testParticipantId, testToken)).to.be.rejectedWith(
  ...
  it("Tokens with incorect signature should fail joining.", function () {
    ...
    return expect(
      portal.join(testParticipantId, tokenWithInvalidSignature)
    ).to.be.rejectedWith("Invalid token signature"); //Note: The error message is not strictly equality checked, instead it will be fullfiled when the actual error message includes the given string here. That is the inherint behavor of 'rejectedWith'.
  });
  ...
  it("token.delete timeout or error should fail joining.", function () {
    ...
    return expect(portal.join(testParticipantId, testToken)).to.be.rejectedWith(
      "timeout or error"
    );
  });
...
describe("portal.leave: Participants leave.", function () {
  it("Before logining should fail.", function () {
    ...
    return portal.leave(testParticipantId).then(
      function (runInHere) {
        expect(runInHere).to.be.false;
      },
      function (err) {
  ...
  it("Should succeed if rpcRequest succeeds.", function () {
    ...
    return portal
      ...
      .then((result) => {
        ...
        expect(mockrpcReq.leave.getCall(0).args).to.deep.equal([
          "rpcIdOfController",
          testParticipantId,
        ]);
      });
...
describe("portal.publish/portal.unpublish/portal.streamControl/portal.onSessionSignaling/portal.subscribe/portal.unsubscribe/portal.updateSubscription", function () {
  it("Before logining should fail.", function () {
    ...
    var pub = portal.publish(testParticipantId, "streamId", {
      type: "webrtc",
      media: { audio: { source: "mic" }, video: { source: "camera" } },
    });
    var unpub = portal.unpublish(testParticipantId, "streamId");
    var sub = portal.subscribe(testParticipantId, "subscriptionId", {
      type: "webrtc",
      media: { audio: { from: "stream1" }, video: { from: "stream2" } },
    });
    var unsub = portal.unsubscribe(testParticipantId, "subscriptionId");
    var streamCtrl = portal.streamControl(testParticipantId, "streamId", {
      operation: "mix",
      data: "common",
    });
    var subscriptionCtrl = portal.subscriptionControl(
      testParticipantId,
      "subscriptionId",
      {
        operation: "update",
        data: { audio: { from: "stream3" }, video: { from: "stream4" } },
      }
    );
    var onSessionSignaling = portal.onSessionSignaling(
      testParticipantId,
      "sessionId",
    ...
    return Promise.all([
      ...
      expect(onSessionSignaling).to.be.rejectedWith(
