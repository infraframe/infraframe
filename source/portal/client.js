...
var Client = function(clientId, sigConnection, portal, version) {
  ...
  const listenAt = (socket) => {
    socket.on('text', function(textReq, callback) {
      ...
      return validator.validate(ReqType.Text, textReq)
        .then((req) => {
    ...
    socket.on('publish', function(pubReq, callback) {
      ...
      var transportId;
      return validator.validate(ReqType.Pub, pubReq)
        .then((req) => {
    ...
    socket.on('stream-control', function(streamCtrlReq, callback) {
      ...
      return validator.validate(ReqType.StreamCtrl, streamCtrlReq)
        .then((req) => {
    ...
    socket.on('subscribe', function(subReq, callback) {
      ...
      var transportId;
      return validator.validate(ReqType.Sub, subReq)
        .then((req) => {
          if (req.transport && req.transport.type == 'quic') {
            req.type = 'quic';
            if (!req.transport.id) {
    ...
    socket.on('subscription-control', function(subCtrlReq, callback) {
      ...
      return validator.validate(ReqType.SubscriptionCtrl, subCtrlReq)
        .then((req) => {
    ...
    socket.on('soac', function(SOAC, callback) {
      ...
      return validator.validate(ReqType.SOAC, SOAC)
        .then((soac) => {
