...
function resolution2WidthHeight(resolution) {
  var r = resolution.indexOf("r"),
    ...
    h =
      r === 0 && x > 1
        ? Number(resolution.substring(x, resolution.length))
        : 480;
  return resolutionName2Value[resolution]
    ? resolutionName2Value[resolution]
    : { width: w, height: h };
}
  ...
  for (var k in dateTime) {
    if (new RegExp("(" + k + ")").test(format)) {
      format = format.replace(
        ...
        RegExp.$1.length == 1
          ? dateTime[k]
          : ("00" + dateTime[k]).substr(("" + dateTime[k]).length)
      );
    }
  }
...
var LegacyClient = function (clientId, sigConnection, portal) {
  ...
  let published = {
    /*StreamId: {type: "webrtc" | "streamingIn", mix: boolean(IfMix)}*/
  }; //FIXME: for compatibility purpose for old clients(such as v3.4.x)
  let ref2subId = {
    /*PeerId | StreamingOutURL | RecorderId: SubscriptionId}*/
  }; //FIXME: for compatibility purpose for old clients(such as v3.4.x)
  let subId2ref = {
    /*SubscriptionId: PeerId | StreamingOutURL | RecorderId}*/
  }; //FIXME: for compatibility purpose for old clients(such as v3.4.x)

  const getViewLabelFromStreamId = (streamId) => {
    var bar_pos = streamId.indexOf("-");
    return bar_pos >= 0 && bar_pos < streamId.length - 1
      ? streamId.substring(bar_pos + 1)
      : streamId;
  };
  ...
  const listenAt = (socket) => {
    socket.on("publish", function (options, url, callback) {
      ...
      return portal
        .publish(clientId, stream_id, pub_info)
    ...
    socket.on("mix", function (options, callback) {
      ...
      return Promise.all(
        options.mixStreams.map((mixStreamId) => {
          var view_label = getViewLabelFromStreamId(mixStreamId) || "common";
          return portal.streamControl(clientId, streamId, {
            operation: "mix",
            data: view_label,
          });
        })
    ...
    socket.on("subscribe", function (options, unused, callback) {
      ...
      options.video &&
        ...
        (sub_desc.media.video.parameters ||
          (sub_desc.media.video.parameters = {})) &&
        (sub_desc.media.video.parameters.resolution = options.video.resolution);
      options.video &&
        typeof options.video.resolution === "string" &&
        (sub_desc.media.video.parameters ||
          (sub_desc.media.video.parameters = {})) &&
        (sub_desc.media.video.parameters.resolution = resolution2WidthHeight(
          options.video.resolution
        ));
      options.video &&
        ...
        (sub_desc.media.video.parameters ||
          (sub_desc.media.video.parameters = {})) &&
        (sub_desc.media.video.parameters.bitrate = qualityLevel2BitrateLevel(
          options.video.quality_level
        ));
      ...
      return portal
        .subscribe(clientId, subscription_id, sub_desc)
        .then(function (result) {
    ...
    socket.on("unsubscribe", function (streamId, callback) {
      ...
      return portal
        .unsubscribe(clientId, subscription_id)
        .then(function () {
          safeCall(callback, "success");
        })
        .catch(function (err) {
    ...
    socket.on(
      ...
      function (message, to_to_deprecated, callback) {
        ...
        var session_id = ref2subId[message.streamId]
        ...
        media: {
          audio:
            options.audio === false
              ? false
              : {
                  from: target_stream_id,
                  format: {
                    codec:
                      options.audio && options.audio.codecs
                        ? options.audio.codecs[0]
                        : "aac",
                  },
                },
          video:
            options.video === false
              ...
              : {
                  ...
                  format: {
                    codec:
                      options.video && options.video.codecs
                        ? options.video.codecs[0]
                        : "h264",
                  },
                },
        },
    ...
    socket.on("updateExternalOutput", function (options, callback) {
      ...
      if (options.resolution) {
        ...
        typeof options.resolution === "string" &&
          (update.video.parameters.resolution = resolution2WidthHeight(
            options.resolution
          ));
      }

      return portal
        .subscriptionControl(clientId, subscription_id, {
          operation: "update",
          data: update,
        })
    ...
    socket.on("startRecorder", function (options, callback) {
      ...
      if (options.recorderId) {
        ...
        return portal
          .subscriptionControl(clientId, subscription_id, update)
          .then((result) => {
            safeCall(callback, "success", {
      ...
      sub_desc.media.video &&
        (sub_desc.media.video.format = { codec: options.videoCodec || "vp8" });
    ...
    socket.on("getRegion", function (options, callback) {
      ...
      var view_label = options.mixStreamId
        ? getViewLabelFromStreamId(options.mixStreamId)
        : "common";
      return portal
        .streamControl(clientId, options.id, {
    ...
    socket.on("mute", function (options, callback) {
      ...
      return portal
        .streamControl(clientId, options.streamId, {
          operation: "pause",
          data: options.track,
        })
        .then(function () {
    ...
    socket.on("customMessage", function (msg, callback) {
      ...
      switch (msg.type) {
        case "data":
          ...
          return portal
            .text(clientId, msg.receiver, msg.data)
            .then(function () {
              safeCall(callback, "success");
            })
            .catch(function (err) {
        ...
        case "control":
          ...
          if (
            ...
            !/^((audio)|(video))-((in)|(out))-((on)|(off))$/.test(
              msg.payload.action
            )
          ) {
          ...
          if (cmdOpts[1] === "out") {
            return portal
              .streamControl(clientId, msg.payload.streamId, {
                operation: operation,
                data: track,
              })
              .then(() => {
  ...
  const convertStreamInfo = (st) => {
    ...
    if (st.media.video) {
      if (st.type === "mixed") {
        ...
        st.media.video.optional &&
          st.media.video.optional.parameters &&
          st.media.video.optional.parameters.resolution &&
          st.media.video.optional.parameters.resolution.forEach(function (
            reso
          ) {
  ...
  const notifyStreamInfo = (streamInfo) => {
    ...
    if (streamInfo.status === "add") {
      ...
      } else if (streamInfo.data.field === "video.layout") {
        //Mixed stream update
  ...
  const notifySessionProgress = (sessionProgress) => {
    ...
    if (sessionProgress.status === "soac") {
      ...
      if (published[id]) {
        ...
        if (published[id].mix) {
          portal
            .streamControl(clientId, id, { operation: "mix", data: "common" })
            .catch((err) => {
  ...
  that.join = (token) => {
    return portal.join(clientId, token).then(function (result) {
