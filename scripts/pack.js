...
optParser.addOption(
  ...
  'target',
  'list',
  'Specify target to pack (Eg. pack.js -t portal -t webrtc-agent)'
);
optParser.addOption(
  'i',
  'install-module',
  'boolean',
  'Whether install node module after packing (Eg. pack.js -t portal -i)'
);
optParser.addOption(
  'r',
  'repack',
  'boolean',
  'Whether clean dist before pack (Eg. pack.js -t portal -r)'
);
optParser.addOption(
  'e',
  'encrypt',
  'boolean',
  'Whether encrypt during pack (Eg. pack.js -t portal -e)'
);
...
optParser.addOption(
  ...
  'addon-debug',
  'boolean',
  'Whether pack debug addon (Eg. pack.js -t webrtc-agent -o)'
);
optParser.addOption(
  'f',
  'full',
  'boolean',
  'Whether perform a full pack (--full is the equalivation of pack.js -t all -r -i). Experimental features are not included, please include them explicitly with -t.'
);
optParser.addOption(
  'p',
  'app-path',
  'string',
  'Specify app path (Eg. pack.js -t all --app-path ${appPath})'
);
optParser.addOption(
  'a',
  'archive',
  'string',
  'Specify archive name (Eg. pack.js -t all -a ${archiveName})'
);
optParser.addOption(
  'n',
  'node-module-path',
  'string',
  'Specify shared-node-module directory'
);
optParser.addOption(
  'c',
  'copy-module-path',
  'string',
  'Specify copy node modules directory'
);
...
function getPackList(targets) {
  if (options.list) {
    // List targets
    console.log('---------------------');
    console.log('Available Targets Are:');
    console.log('---------------------');
  ...
  var packList = targets.filter((element) => {
    ...
    if (
      options.target.includes('all') &&
      !experimentalTargets.includes(element.rules.name)
    )
      return true;
...
function cleanIfRepack(targets) {
  if (options.repack) {
    return exec(`rm -rf ${distDir}`).then((ret) => {
      return targets;
    });
  }
  return targets;
...
function processTargets(targets) {
  return exec(`mkdir -p ${distDir}`).then((mkdir) => {
    packings = [];
    for (const target of targets) {
      packings.push(packTarget(target));
    }
    return Promise.all(packings).then(() => {
      return targets;
    });
  });
}
...
function packTarget(target) {
  ...
  return exec(`mkdir -p ${packDist}`)
    ...
    .then((addonPacked) => {
      ...
      if (options.encrypt) return encrypt(target);
    });
}
  ...
  if (common.files) {
    ...
    for (const file of common.files) {
      ...
      if (options.lint && extname === '.js') {
        try {
          execSync(
            `${rootDir}/node_modules/.bin/eslint -c ${rootDir}/source/.eslintrc.json ${filePath}`
          );
        } catch (error) {
  ...
  if (common.package) {
    ...
    if (options['copy-module-path']) {
      ...
      if (options['node-module-path']) {
        ...
        chdir(installDist);
        npmInstall = exec('npm install' + npmInstallOption).then(
          (stdout, stderr) => {
...
function packAddon(target) {
  ...
  for (const addon of natives.addons) {
    ...
    let addonPath = path.join(
      packSrc,
      addon.folder,
      `build/${buildType}/${addon.name}.node`
    );
    ...
    // Get addon lib files
    let packLib = getAddonLibs(path.join(addonDist, `${addon.name}.node`)).then(
      (libs) => {
  ...
  return Promise.all(packLibs).then((libsOk) => {
    // Replace openh264 if needed
    let libOpenh264 = path.join(libDist, 'libopenh264.so.8');
    if (
      options['archive'] &&
      fs.existsSync(libOpenh264) &&
      !options['no-pseudo']
    ) {
      let dummyOpenh264 = path.join(
        rootDir,
        'third_party/openh264/pseudo-openh264.so'
      );
    ...
    if (
      options['archive'] &&
      fs.existsSync(libSvtHevcEnc) &&
      !options['no-pseudo']
    ) {
      let dummySvtHevcEnc = path.join(
        rootDir,
        'third_party/SVT-HEVC/pseudo-svtHevcEnc.so'
      );
      execSync(`cp -av ${dummySvtHevcEnc} ${libSvtHevcEnc}`);
    }

    if (
      target.rules.name.indexOf('video') === 0 ||
      ...
      if (fs.existsSync('/opt/intel/mediasdk')) {
        ...
        execSync(
          `find /opt/intel/mediasdk -type f -name metrics_monitor  | xargs -I '{}' cp -av '{}' ${dst_bin}`
        );
        execSync(
          `find /opt/intel/mediasdk -type f -name libcttmetrics.so | xargs -I '{}' cp -av '{}' ${dst_lib}`
        );
      }
    }
...
function getAddonLibs(addonPath) {
  ...
  return exec(
    `ldd ${addonPath} | grep '=>' | awk '{print $3}' | sort | uniq | grep -v "^("`,
    { env }
  )
    .then((stdout, stderr) => {
      var checks = [];
      for (let line of stdout.trim().split('\n')) {
        ...
        checkPros[line] = exec(`[ -s ${line} ]`)
          .then((ok) => {
            ...
            if (osType.includes('ubuntu')) {
              return exec(`dpkg -S ${line} 2>\/dev\/null`, { env })
                .then((out, err) => {
                  if (out.includes('libc6') || out.includes('libselinux'))
                    return '';
                  return line;
                })
                .catch((e) => line);
          ...
          .catch((e) => {
            ...
            if (!line.startsWith('/')) {
              return exec(`ldd ${addonPath} | grep '=>' | grep -v '=> /'`).then(
                (stdout) => {
                  e.message =
                    `library dependency not found for\n  ${addonPath}:\n${stdout}` +
...
function isLibAllowed(libSrc) {
  ...
  if (!options['archive'] || options['with-ffmpeg']) {
    ...
    if (
      osType.includes('centos') ||
      (osType.includes('ubuntu') && osType.includes('20.04'))
    ) {
...
function generateStart(target) {
  ...
  if (options['node-module-path']) {
    content +=
      'NODE_PATH=' + path.resolve(options['node-module-path']) + ' \\\n';
  }
...
function encrypt(target) {
  ...
  return exec(
    ...
    .then(() => {
      // Encrypt Node
      var nodeList = execSync(
        `find ${packDist} -type f -name "*.node"`
      ).toString();
      var nodeJobs = nodeList
        .trim()
        .split('\n')
        .map((nodeFile) => {
          if (!nodeFile) return;
...
function packScripts() {
  ...
  execSync(
    `cp -a ${rootDir}/scripts/release/package.mcu.json ${distDir}/package.json`
  );
  ...
  if (options.binary) {
    execSync(
      `cp -a ${rootDir}/scripts/release/daemon-bin.sh ${binDir}/daemon.sh`
    );
  } else {
    execSync(
      `cp -a ${rootDir}/scripts/release/daemon-mcu.sh ${binDir}/daemon.sh`
    );
  }
  ...
  const baseContent = fs.readFileSync(
    `${rootDir}/scripts/release/launch-base.sh`
  );
...
function archive() {
  ...
  execSync(
    `tar --numeric-owner -czf "Release-${myVersion}.tgz" Release-${myVersion}/`
  );
  execSync(`mv Release-${myVersion} ${distDir}`);
  console.log(
    `\x1b[32mRelease-${myVersion}.tgz generated in ${rootDir}.\x1b[0m`
  );
}
