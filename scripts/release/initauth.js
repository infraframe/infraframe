...
const updateRabbit = (cleanup) =>
  new Promise((resolve, reject) => {
    if (cleanup) {
      console.log("Cleanup RabbitMQ account...");
      saveAuth({ rabbit: null }, authStore).then(resolve).catch(reject);
      return;
    }
    question("Update RabbitMQ account?[yes/no]").then((answer) => {
      answer = answer.toLowerCase();
      if (answer !== "y" && answer !== "yes") {
        resolve();
        return;
      }
      question(`(${authBase}) Enter username of rabbitmq: `).then(
        (username) => {
          question(`(${authBase}) Enter password of rabbitmq: `).then(
            (password) => {
...
const updateMongo = (cleanup) =>
  new Promise((resolve, reject) => {
    if (cleanup) {
      console.log("Cleanup MongoDB account...");
      saveAuth({ mongo: null }, authStore).then(resolve).catch(reject);
      return;
    }
    question("Update MongoDB account?[yes/no]").then((answer) => {
      answer = answer.toLowerCase();
      if (answer !== "y" && answer !== "yes") {
        resolve();
        return;
      }
      question(`(${authBase}) Enter username of mongodb: `).then((username) => {
        question(`(${authBase}) Enter password of mongodb: `).then(
          (password) => {
            mutableStdout.muted = false;
...
const updateInternal = (cleanup) =>
  new Promise((resolve, reject) => {
    if (cleanup) {
      console.log("Cleanup internal passphrase...");
      saveAuth({ internalPass: null }, authStore).then(resolve).catch(reject);
      return;
    }
    question("Update internal passphrase?[yes/no]").then((answer) => {
      answer = answer.toLowerCase();
      if (answer !== "y" && answer !== "yes") {
        resolve();
        return;
      }
      question(`(${authBase}) Enter internal passphrase: `).then(
        (passphrase) => {
          mutableStdout.muted = false;
...
const updateGKeyPass = (cleanup) =>
  new Promise((resolve, reject) => {
    if (cleanup) {
      console.log("Cleanup gRPC TLS key...");
      saveAuth({ grpc: null }, authStore).then(resolve).catch(reject);
      return;
    }
    question("Update passphrase for gRPC TLS key?[yes/no]").then((answer) => {
      answer = answer.toLowerCase();
      if (answer !== "y" && answer !== "yes") {
        resolve();
        return;
      }
      question(`(${authBase}) Enter passphrase of server key: `).then(
        (serverPass) => {
          mutableStdout.muted = false;
          question(`(${authBase}) Enter passphrase of client key: `).then(
            (clientPass) => {
              mutableStdout.muted = false;
...
const generateServiceProtectionKey = (cleanup) =>
  new Promise((resolve, reject) => {
    if (cleanup) {
      console.log("Cleanup service protection key...");
      saveAuth({ spk: null }, authStore).then(resolve).catch(reject);
      return;
    }
    question("Generate service protection key?[yes/no]").then((answer) => {
      answer = answer.toLowerCase();
      if (answer === "y" || answer === "yes") {
